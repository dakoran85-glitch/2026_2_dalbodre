...
const INITIAL_DB = {
  ...
  pendingShopItems: [],
  funding: [],
  purchaseHistory: [],   // ⭐ V10.4 수정: purchaseHistory 초기화 포함
  roleExp: {},
  bonusCoins: {},
  usedCoins: {},
  penaltyCount: {},
...
export default function App() {
  ...
  const sync = async (updates) => {
    ...
    try {
      await fetch(`${DATABASE_URL}${DB_PATH}.json`, { method: 'PATCH', body: JSON.stringify(updates) });
    } catch (_) {}
  };
  ...
  const startStopwatch = () => {
    sync({ timer: { mode: 'class_sw', startedAt: Date.now(), endTime: null, duration: null, isRunning: true, pausedElapsed: null, pausedRemaining: null } });
  };

  const startCountdown = (minutes) => {
    const ms = minutes * 60 * 1000;
    sync({ timer: { mode: 'class_cd', startedAt: Date.now(), endTime: Date.now() + ms, duration: ms, isRunning: true, pausedElapsed: null, pausedRemaining: null } });
  };

  const startBreak = (minutes) => {
    const ms = minutes * 60 * 1000;
    sync({ timer: { mode: 'break', startedAt: Date.now(), endTime: Date.now() + ms, duration: ms, isRunning: true, pausedElapsed: null, pausedRemaining: null } });
    playSound('chime');
  };
  ...
  const pauseTimer = () => {
    ...
    if (t.mode === 'class_sw') {
      const elapsed = Date.now() - t.startedAt + (t.pausedElapsed || 0);
  ...
  const resumeTimer = () => {
    ...
    if (!t || t.isRunning) return;
    if (t.mode === 'class_sw') {
      sync({ timer: { ...t, isRunning: true, startedAt: Date.now() - (t.pausedElapsed || 0), pausedElapsed: null } });
    } else {
      const remaining = t.pausedRemaining || 0;
      sync({ timer: { ...t, isRunning: true, endTime: Date.now() + remaining, pausedRemaining: null } });
    }
  };
  ...
  const toggleAttendance = (sid) => {
    ...
    const newAttendance = {
      ...db.attendance,
      [weekKey]: { ...(db.attendance?.[weekKey] || {}), [sid]: newDays }
    };
    // 개근 체크
    const totalCount = Math.min(5, newDays.length + (db.extraAttendDays || 0));
    let bonusUpdates = {};
    if (totalCount >= 5 && !db.attendanceBonus?.[weekKey]?.[sid]) {
      const newBonus = { ...(db.bonusCoins || {}), [sid]: (db.bonusCoins?.[sid] || 0) + 3 };
      const newAttBonus = { ...(db.attendanceBonus || {}), [weekKey]: { ...(db.attendanceBonus?.[weekKey] || {}), [sid]: true } };
      bonusUpdates = { bonusCoins: newBonus, attendanceBonus: newAttBonus };
      playSound('jackpot');
    }
    sync({ attendance: newAttendance, ...bonusUpdates });
  };
  ...
  const teacherAddHoliday = () => {
    ...
    let bonusUpdates = { bonusCoins: { ...db.bonusCoins }, attendanceBonus: { ...db.attendanceBonus } };
    allStats.forEach(s => {
      const attended = safeArray(db.attendance?.[weekKey]?.[s.id]).length;
      const newTotal = Math.min(5, attended + newExtra);
      if (newTotal >= 5 && !db.attendanceBonus?.[weekKey]?.[s.id]) {
        bonusUpdates.bonusCoins[s.id] = (bonusUpdates.bonusCoins[s.id] || 0) + 3;
        if (!bonusUpdates.attendanceBonus[weekKey]) bonusUpdates.attendanceBonus[weekKey] = {};
        bonusUpdates.attendanceBonus[weekKey][s.id] = true;
      }
    });
    sync({ extraAttendDays: newExtra, ...bonusUpdates });
    alert(`공휴일 보정 완료! 이번 주 보정값: +${newExtra}일`);
  };

  // ── 온기 우체통 (칭찬) ──────────────────────────────────
  const submitPraise = () => {
    if (!praiseTarget || !praiseTag || !praiseText.trim()) {
      alert("대상, 역량, 내용을 모두 입력해 주세요.");
      return;
    }
    const isTheme = praiseTag === db.settings?.weeklyTheme;
    const basicCoins = db.settings?.pointConfig?.praiseBasic || 10;
    const bonusCoins = db.settings?.pointConfig?.praiseBonus || 15;
    const totalCoins = isTheme ? basicCoins + bonusCoins : basicCoins;
    const newPraise = {
      id: Date.now(),
      toId: praiseTarget === 'me' ? 'me' : toInt(praiseTarget),
      tag: praiseTag,
      text: praiseText.trim(),
      date: formatDate(),
      coins: totalCoins
    };
    // 칭찬 받은 학생 코인 지급
    let updates = { approvedPraises: [...safeArray(db.approvedPraises), newPraise] };
    if (praiseTarget !== 'me') {
      updates.bonusCoins = { ...db.bonusCoins, [praiseTarget]: (db.bonusCoins?.[praiseTarget] || 0) + totalCoins };
    }
    sync(updates);
    playSound('good');
    alert(`온기 우체통 전달 완료! 🌸 ${isTheme ? `(테마 일치 보너스 +${bonusCoins}🪙 포함)` : ''}`);
    setShowPraiseModal(false);
    setPraiseTarget(""); setPraiseTag(""); setPraiseText("");
  };

  // ── 성찰 제출 ────────────────────────────────────────────
  const submitReflection = () => {
    if (!refTarget || !refTag || !refText.trim()) {
      alert("대상, 역량, 다짐을 모두 입력해 주세요.");
      return;
    }
    const newRef = { id: Date.now(), studentId: toInt(refTarget), tag: refTag, text: refText.trim(), date: formatDate() };
    sync({
      studentStatus: { ...db.studentStatus, [refTarget]: 'pending' },
      pendingReflections: [...safeArray(db.pendingReflections), newRef]
    });
    playSound('good');
  ...
  const handleGivePenalty = (sid) => {
    if (!window.confirm("이 학생을 위기 상태로 지정할까요?\n성찰과 회복 센터에서 다짐을 작성해야 해제됩니다.")) return;
    const penaltyUnit = db.settings?.pointConfig?.penalty || 20;
    sync({
  ...
  const handleExpAdjust = (sid, delta) => {
    const cur = db.roleExp[sid] || 0;
    const newVal = Math.max(0, cur + delta);
    const newRoleExp = { ...db.roleExp, [sid]: newVal };
    let updates = { roleExp: newRoleExp };
    if (delta > 0) {
      updates.allTime = { ...db.allTime, exp: { ...(db.allTime?.exp || {}), [sid]: (db.allTime?.exp?.[sid] || 0) + delta } };
    }
    sync(updates);
  ...
  const handleFund = (fundId, sid, amount) => {
    ...
    if (!user) return;
    if (user.coins < amount) return alert("코인이 부족합니다.");
    const fund = safeArray(db.funding).find(f => f.id === fundId);
    if (!fund) return;
    if (!window.confirm(`${user.name}의 코인 ${amount}🪙을 "${fund.name}" 펀딩에 투자할까요?`)) return;
    const newFunding = safeArray(db.funding).map(f => f.id === fundId ? { ...f, current: toInt(f.current) + amount } : f);
    sync({
      funding: newFunding,
      usedCoins: { ...db.usedCoins, [sid]: (db.usedCoins[sid] || 0) + amount },
  ...
  const submitArtisanItem = () => {
    if (!artisanTarget || !artisanItemName.trim() || !artisanItemPrice) {
      alert("장인, 아이템 이름, 가격을 모두 입력하세요.");
      return;
    }
    ...
    const newItem = {
      id: Date.now(),
      name: artisanItemName.trim(),
      price: toInt(artisanItemPrice),
      creator: artisan.name,
      creatorId: artisan.id
    };
    sync({ pendingShopItems: [...safeArray(db.pendingShopItems), newItem] });
    alert(`"${newItem.name}" 아이템이 결재 요청되었습니다! 선생님의 승인을 기다려 주세요.`);
    setArtisanTarget(""); setArtisanItemName(""); setArtisanItemPrice("");
  };

  // ── 공동 퀘스트 점수 추가 ───────────────────────────────
  const addCoopScore = (amount, name) => {
    sync({ manualRepOffset: (db.manualRepOffset || 0) + amount });
    playSound('good');
  };

  // ── 사이좋은 일주일 조정 ─────────────────────────────────
  const adjustGoodWeek = (delta) => {
    const cur = db.coopQuest?.goodWeek || 0;
    const newVal = Math.max(0, Math.min(5, cur + delta));
    sync({ coopQuest: { ...db.coopQuest, goodWeek: newVal } });
  };
  ...
  const completeGoodWeek = () => {
    ...
    playSound('jackpot');
    alert(`🌟 사이좋은 일주일 달성! 학급 명성 +${reward}p 획득!`);
  };
  ...
  const handleCompleteTimeAttack = () => {
    const reward = db.timeAttack?.rewardRep || 100;
    sync({
      timeAttack: { isActive: false, title: "", rewardRep: 100, endTime: null, cleared: [] },
      manualRepOffset: (db.manualRepOffset || 0) + reward
    });
    playSound('jackpot');
    alert(`🎉 타임어택 성공! 학급 명성 +${reward}p 획득!`);
  };

  const handleFailTimeAttack = () => {
    sync({ timeAttack: { isActive: false, title: "", rewardRep: 100, endTime: null, cleared: [] } });
    playSound('bad');
    alert("타임어택이 종료되었습니다.");
  };
  ...
  const toggleTimeAttackClear = (sid) => {
    ...
    const newCleared = cleared.includes(Number(sid)) ? cleared.filter(id => id !== Number(sid)) : [...cleared, Number(sid)];
    sync({ timeAttack: { ...db.timeAttack, cleared: newCleared } });
  };
  ...
  const handleAddStudent = () => {
    ...
    const newS = { id: maxId + 1, name: newStudentName.trim(), role: '', group: toInt(newStudentGroup, 1), isLeader: false, enneagram: newStudentEnneagram };
    sync({ students: [...safeStudents, newS] });
    setNewStudentName(""); setNewStudentGroup("1"); setNewStudentEnneagram("");
    alert(`${newS.name} 학생이 추가되었습니다.`);
  };

  const handleRemoveStudent = (sid) => {
    const s = safeStudents.find(s => s.id === sid);
    if (!s) return;
    if (!window.confirm(`${s.name} 학생을 명단에서 삭제할까요?`)) return;
    sync({ students: safeStudents.filter(s => s.id !== sid) });
  };

  const handleStudentFieldChange = (sid, field, value) => {
    const newStudents = safeStudents.map(s => s.id === sid ? { ...s, [field]: value } : s);
    sync({ students: newStudents });
  };
  ...
  const submitNote = () => {
    ...
    const newNote = { id: Date.now(), text: noteText.trim(), date: formatDate() };
    const existingNotes = safeArray(db.notes?.[showNoteModal]);
    sync({ notes: { ...db.notes, [showNoteModal]: [...existingNotes, newNote] } });
  ...
  const deleteNote = (sid, noteId) => {
    ...
    const existingNotes = safeArray(db.notes?.[sid]).filter(n => n.id !== noteId);
    sync({ notes: { ...db.notes, [sid]: existingNotes } });
  };
  ...
  const exportStudent = (sid) => {
    ...
    const reflections = safeArray(db.pendingReflections).filter(r => r.studentId == s.id);
    const lines = [
      `=== ${s.name} 학생 SEL 리포트 (${formatDate()}) ===`,
      `에니어그램: ${s.enneagram ? ENNEAGRAM_DATA[s.enneagram]?.name : '미입력'}`,
      `직업 숙련도: ${s.exp}회 (${s.mastery.label})`,
      `보유 코인: ${s.coins}🪙`,
      `이번 주 출석: ${s.weeklyCount}/5`,
      `누적 개근 연속: ${s.streak}주`,
      `누적 위기 횟수: ${s.atPen}회`,
      `누적 기부: ${s.atDonate}🪙`,
      ``,
      `[온기 칭찬 내역 (${praises.length}건)]`,
      ...praises.map(p => `- [${SEL_OPTIONS.find(o=>o.name===p.tag)?.short}] "${p.text}" (${p.date})`),
      ``,
      `[성찰 다짐 내역 (${reflections.length}건)]`,
      ...reflections.map(r => `- [${SEL_OPTIONS.find(o=>o.name===r.tag)?.short}] "${r.text}" (${r.date})`),
      ``,
      `[누가기록 (${s.notes.length}건)]`,
      ...s.notes.map(n => `- (${n.date}) ${n.text}`),
    ];
    const text = lines.join('\n');
    navigator.clipboard?.writeText(text).then(() => alert(`${s.name} 리포트가 클립보드에 복사되었습니다!`)).catch(() => alert(text));
  };
  ...
  const exportAll = () => {
    const lines = allStats.map(s => {
      ...
      return [
        `\n=== ${s.name} ===`,
        `에니어그램: ${s.enneagram ? ENNEAGRAM_DATA[s.enneagram]?.name : '미입력'}`,
        `숙련도: ${s.exp}회 | 코인: ${s.coins}🪙 | 출석: ${s.weeklyCount}/5 | 위기: ${s.atPen}회`,
        `칭찬(${praises.length}건): ${praises.map(p=>`"${p.text}"`).join(', ')}`
      ].join('\n');
    });
    const text = `달보드레 전체 리포트 (${formatDate()})\n` + lines.join('\n');
    navigator.clipboard?.writeText(text).then(() => alert("전체 반 리포트가 클립보드에 복사되었습니다!")).catch(() => alert(text));
  };

  // ── 학기 마감 / 초기화 ──────────────────────────────────
  const closeSemester = () => {
    if (window.prompt("1학기 마감을 실행하려면 '마감'을 입력하세요.") !== "마감") return;
    sync({
      roleExp: {},
      bonusCoins: {},
      usedCoins: {},
    ...
    alert("1학기 마감이 완료되었습니다. 누적 데이터는 보존됩니다.");
  };

  const factoryReset = () => {
    if (window.prompt("모든 데이터를 초기화하려면 '전체초기화'를 입력하세요.") !== "전체초기화") return;
    sync({
      roleExp: {},
      bonusCoins: {},
      usedCoins: {},
          ...
          <div className="lg:col-span-7 flex flex-col gap-10 w-full justify-between py-2">
            <div className="flex flex-row items-center justify-start gap-12 pl-2">
              <div className="flex items-baseline">
                <span className="text-[140px] md:text-[180px] font-black text-[#6B4423] drop-shadow-md tracking-tighter leading-[0.85]">{classReputation}</span>
              ...
              <div className="text-left space-y-10 bg-emerald-50/60 p-10 md:p-12 rounded-[40px] border-2 border-emerald-100 shadow-inner relative z-10">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {SEL_OPTIONS.map(opt => (
                      <button key={opt.id} onClick={() => setRefTag(opt.name)} className={`p-5 rounded-2xl border-2 font-black text-sm text-left transition-all shadow-sm ${refTag === opt.name ? 'bg-emerald-500 border-emerald-500 text-white scale-105' : 'bg-white border-white text-slate-500 hover:-translate-y-1'}`}>
                        {opt.name}
                      </button>
                    ))}
                  </div>
                      ...
                      <div key={item.id} className="bg-white p-10 rounded-[40px] shadow-sm border-2 border-slate-100 flex flex-col justify-between hover:border-amber-300 transition-colors">
                        ...
                        <div className="flex gap-4">
                          ...
                          <button onClick={() => {
                            ...
                            const newHistory = { id: Date.now(), date: formatDate(), itemName: String(item.name), buyerName: user.name, price: price };
                            let updates = {
                              usedCoins: { ...db.usedCoins, [sid]: (db.usedCoins[sid] || 0) + price },
                              purchaseHistory: [newHistory, ...safeArray(db.purchaseHistory)].slice(0, 50)
                            };
                            ...
                            if (item.creatorId) {
                              ...
                              alertMsg = `결제 완료! 💸\n장인(${item.creator})에게 로열티 +${coinBonus}🪙, 학급 명성 +${repBonus}p!`;
                            }
                            sync(updates);
                            alert(alertMsg);
                            playSound('buy');
                          ...
                          return (
                            <div key={r.id} className="bg-white p-6 rounded-3xl border border-emerald-100 flex justify-between items-start gap-4 shadow-sm">
                              ...
                              <div className="flex gap-2 shrink-0">
                                <button onClick={() => {
                                  sync({
                                    studentStatus: { ...db.studentStatus, [r.studentId]: 'normal' },
                                    pendingReflections: safeArray(db.pendingReflections).filter(x => x.id !== r.id)
                                  });
                                  playSound('good');
                                  alert(`${s?.name} 학생의 성찰이 승인되어 위기에서 탈출했습니다! 🌱`);
                                }} className="bg-emerald-500 text-white px-5 py-3 rounded-2xl font-black text-sm hover:bg-emerald-600 shadow-md">승인</button>
                                <button onClick={() => {
                                  sync({ pendingReflections: safeArray(db.pendingReflections).filter(x => x.id !== r.id) });
                        ...
                        {safeArray(db.pendingShopItems).map(item => (
                          <div key={item.id} className="bg-white p-6 rounded-3xl border border-amber-100 flex justify-between items-center gap-4 shadow-sm">
                            ...
                            <div className="flex gap-2 shrink-0">
                              <button onClick={() => {
                                sync({
                                  shopItems: [...safeArray(db.shopItems), item],
                                  pendingShopItems: safeArray(db.pendingShopItems).filter(x => x.id !== item.id)
                                });
                                alert(`"${item.name}" 아이템이 상점에 등록되었습니다!`);
                                playSound('good');
                              }} className="bg-amber-500 text-white px-5 py-3 rounded-2xl font-black text-sm hover:bg-amber-600 shadow-md">승인</button>
                              <button onClick={() => {
                                sync({ pendingShopItems: safeArray(db.pendingShopItems).filter(x => x.id !== item.id) });
                              }} className="bg-slate-200 text-slate-600 px-5 py-3 rounded-2xl font-black text-sm hover:bg-slate-300">반려</button>
                            </div>
                          </div>
                        ))}
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 space-y-8">
                    ...
                    <div className="pt-8 border-t border-slate-200 mt-8">
                      ...
                      <div className="flex gap-3 mb-6 bg-blue-50 p-6 rounded-3xl border border-blue-100">
                        <input id="new_fund_name_input" type="text" placeholder="새로운 펀딩 목표 (예: 피자 파티)" className="flex-1 p-4 rounded-xl border border-blue-200 font-bold outline-none focus:border-blue-400"/>
                            ...
                            <div className="border-t-2 border-slate-200 pt-8 mt-8">
                              ...
                              {showNotesInReport && (
                                <div className="mt-4 space-y-3 animate-in fade-in">
                                  ...
                                  <button onClick={() => openNoteModal(s.id)} className="w-full bg-yellow-500 text-white p-4 rounded-2xl font-black text-base flex items-center justify-center gap-2 hover:bg-yellow-600 shadow-md mt-4">
                ...
                <div className="space-y-8 animate-in fade-in">
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 mb-8">
                    ...
                    <div className="flex gap-3 mb-6 bg-indigo-50 p-6 rounded-3xl border border-indigo-100">
                      ...
                      }} className="bg-indigo-600 text-white px-10 rounded-xl font-black text-lg shadow-md hover:bg-indigo-700 active:scale-95 transition-transform">목록에 추가</button>
                    </div>
                    <div className="flex flex-wrap gap-3">
                      {safeArray(db.rolesList).map((r, idx) => (
                        <div key={idx} className="flex items-center gap-3 bg-white px-5 py-3 rounded-2xl border-2 border-indigo-100 shadow-sm hover:border-indigo-300 transition-colors">
                          ...
                          }} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 className="w-5 h-5"/></button>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="space-y-8 animate-in fade-in">
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100">
                    <div className="bg-amber-50 p-8 rounded-3xl border-2 border-amber-200 mb-10 shadow-sm">
                      ...
                      <button onClick={teacherAddHoliday} className="w-full bg-amber-500 text-white py-5 rounded-2xl font-black text-xl hover:bg-amber-600 shadow-md transition-transform active:scale-95">
                        전체 학생의 출석일수 +1일 반영하기
                      </button>
                    </div>
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 space-y-10">
                    ...
                    <div className="pt-8 border-t-2 border-slate-100">
                      <button onClick={toggleCumulativeStats} className={`w-full py-5 rounded-2xl font-black text-xl transition-all shadow-md flex items-center justify-center gap-3 ${db.settings?.showCumulativeStats ? 'bg-blue-600 text-white' : 'bg-white text-slate-500 border-2 border-slate-300 hover:border-blue-300'}`}>
                        <Eye className="w-6 h-6"/> 누적 스탯 표시 (교사 모드): {db.settings?.showCumulativeStats ? 'ON' : 'OFF'}
                      </button>
                    </div>
                    {/* 개별 학생 코인 강제 조정 */}
                      ...
                      <div className="flex flex-col md:flex-row gap-4 mb-10 bg-white p-6 rounded-3xl shadow-sm border border-indigo-200">
                        ...
                        <button onClick={() => {
                          ...
                          if (window.confirm(`${v > 0 ? '+'+v : v}점의 명성을 즉시 적용할까요?`)) {
                            ...
                            setManualScoreInput(""); playSound('good');
                          }
                        }} className="bg-indigo-600 text-white px-10 rounded-2xl font-black text-lg shadow-md hover:bg-indigo-700 active:scale-95 transition-transform">명성 적용</button>
                      </div>
                ...
                <div className="animate-in fade-in space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className="bg-emerald-50 border-4 border-emerald-200 p-10 rounded-[40px] text-center shadow-lg flex flex-col justify-between">
                      ...
                      <button onClick={() => {
                        if (window.prompt("직업 숙련도를 0으로 초기화하려면 '직업초기화'를 입력하세요.") === "직업초기화") {
                          sync({ roleExp: {} });
                          alert("직업 점수가 모두 0으로 초기화되었습니다.");
                        }
                      }} className="w-full bg-emerald-600 text-white py-5 rounded-[24px] font-black text-xl shadow-xl hover:bg-emerald-700 active:scale-95 transition-all">직업 초기화 실행</button>
                    </div>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-[5001] p-4">
          <div className="bg-white p-12 rounded-[50px] w-full max-w-xl shadow-2xl animate-in zoom-in-95 border-4 border-pink-100 relative overflow-hidden">
            ...
            <div className="flex gap-4 relative z-10">
              ...
              <button onClick={submitPraise} className="flex-1 py-6 bg-pink-500 text-white rounded-[30px] font-black text-xl shadow-xl hover:bg-pink-600 active:scale-95 transition-transform flex justify-center items-center gap-2">
          ...
          <div className="fixed inset-0 bg-white z-[5004] overflow-auto flex flex-col items-center">
            ...
            <div className="max-w-5xl w-full p-16 print:p-0">
              ...
              <div className="grid grid-cols-2 gap-8 print:grid-cols-2 print:gap-6">
                ...
                {praises.length === 0 && (
                  <div className="col-span-2 text-center py-32 text-slate-300 font-black text-3xl">아직 받은 사연이 없습니다. 친구에게 먼저 온기를 건네볼까요?</div>
                )}
              </div>
            </div>
          </div>
