                        ...
                        <div className="bg-purple-50 p-6 md:p-8 rounded-[30px] border-2 border-purple-200 shadow-sm flex flex-col">
                           ...
                           <div className="space-y-3 mb-6 max-h-[150px] overflow-y-auto pr-2 custom-scrollbar">
                              {safeArray(marketPresets).map(m => {
                                if(!m) return null;
                                return (
                                <div key={m.id} className={`p-4 md:p-5 rounded-2xl border-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 transition-all ${activeMarket?.id === m.id ? 'bg-purple-600 border-purple-500 text-white shadow-lg scale-105 my-2' : 'bg-white border-purple-100 hover:border-purple-300'}`}>
                                   <div className="w-full sm:w-auto">
