...
import { 
  ...
  Zap, Crown, Gift, Coins, BarChart3, Volume2, MessageCircleHeart
} from 'lucide-react';
...
export default function App() {
  ...
  const [db, setDb] = useState({
    ...
    settings: { 
      ...
      targetScore: 3000, forceShopOpen: false, weeklyTheme: "4단계: 관계 기술 (Relationship skills)", 
    ...
    pendingShopItems: [], // 🔨 장인 건의 아이템 대기열
    ...
    activeMission: { isActive: false, participants: [] } // 🎮 참여형 긴급 미션 상태
  });
  ...
  useEffect(() => {
    const fetchLive = async () => {
      try { 
        const res = await fetch(`${DATABASE_URL}v7Data.json`); 
        ...
        if (data) {
          setDb(prev => ({
            ...
            activeMission: data.activeMission || { isActive: false, participants: [] },
            pendingShopItems: safeArray(data.pendingShopItems)
          })); 
        }
      } catch (e) {}
      setIsLoading(false);
  ...
  const sync = async (updates) => {
    ...
    try { await fetch(`${DATABASE_URL}v7Data.json`, { method: 'PATCH', body: JSON.stringify(updates) }); } catch (e) {}
  };
  ...
  // 🔥 교사 토글에 따른 정렬 로직 분리 (번호순 vs 위기순)
  const sortedDashboardStats = useMemo(() => {
    if (db.settings.showCumulativeStats) {
      return [...allStats].sort((a, b) => a.id - b.id); // 토글 켜지면 번호순 정렬
    } else {
  ...
  const handleGivePenalty = (id) => {
    ...
    if (window.confirm("위기 상태(성찰 요망)로 지정할까요?")) {
      playSound('bad');
  ...
  const handleDonate = (sId, amount) => {
    ...
    if (!user || user.coins < amount) return alert("코인이 부족해요 😥");
    playSound('buy');
    sync({ usedCoins: { ...db.usedCoins, [sId]: (db.usedCoins[sId] || 0) + amount }, donations: [{ id: Date.now(), name: user.name, amount }, ...safeArray(db.donations)].slice(0, 15), allTime: { ...db.allTime, donate: { ...db.allTime.donate, [sId]: (db.allTime.donate?.[sId] || 0) + amount } } });
    alert("멋진 기부 감사합니다! ✨");
  };
  ...
  const handleFund = (fId, sId, amount) => {
    ...
    if (!user || user.coins < amount) return alert("코인이 부족해요 😥");
    playSound('buy');
    sync({ usedCoins: { ...db.usedCoins, [sId]: (db.usedCoins[sId] || 0) + amount }, funding: safeArray(db.funding).map(f => f.id === fId ? { ...f, current: f.current + amount } : f), allTime: { ...db.allTime, fund: { ...db.allTime.fund, [sId]: (db.allTime.fund?.[sId] || 0) + amount } } });
    alert("투자 성공! 목표 달성까지 화이팅! 🚀");
  };
  ...
  const submitPraise = () => {
    ...
    alert("따뜻한 온기가 배달되었어요! 💌"); playSound('good');
  };
  ...
  const submitReflection = () => {
    ...
    alert("용기 있는 다짐이 제출되었습니다! 선생님께 확인받으세요."); playSound('good');
  };

  // 🎮 참여형 긴급 미션 핸들러
  const startMission = () => {
    if(window.confirm("참여형 긴급 미션을 발동하시겠습니까? (성공 시 1인당 1점 부여)")) {
      sync({ activeMission: { isActive: true, participants: [] } });
      alert("미션 발동!"); playSound('gacha');
    }
  };

  const endMission = () => {
    if(window.confirm("미션을 강제 종료할까요? (현재까지 완료한 점수만 반영됩니다)")) {
      sync({ activeMission: { isActive: false, participants: [] } });
    }
  };

  const participateMission = (sId) => {
    if(db.activeMission.participants.includes(sId)) return;
    playSound('good');
    const nextParticipants = [...db.activeMission.participants, sId];
    let nextOffset = (db.manualRepOffset || 0) + 0.5; // 1명당 0.5점 즉시 반영
    let nextMissionState = { isActive: true, participants: nextParticipants };

    // 전원 성공 시!
    if (nextParticipants.length >= safeStudents.length) {
      playSound('jackpot');
      alert(`🎉 전원 미션 성공!! 보너스 점수(+${safeStudents.length * 0.5}p)가 지급되었습니다!`);
      nextOffset += (safeStudents.length * 0.5); // 나머지 절반의 보너스 부여 (총 1명당 1점)
      nextMissionState.isActive = false; // 자동 종료
    }
    sync({ manualRepOffset: nextOffset, activeMission: nextMissionState });
  };

  // 🔨 장인 아이템 건의 핸들러
  const submitArtisanItem = () => {
    if(!artisanTarget || !artisanItemName || !artisanItemPrice) return alert("정보를 모두 입력하세요.");
    const artisan = allStats.find(s => s.id == artisanTarget);
    if(!artisan || artisan.exp < 20) return alert("장인만 등록 가능합니다.");
    const newItem = { id: Date.now(), name: artisanItemName, price: parseInt(artisanItemPrice), creator: artisan.name };
    sync({ pendingShopItems: [newItem, ...safeArray(db.pendingShopItems)] });
    setArtisanTarget(""); setArtisanItemName(""); setArtisanItemPrice("");
    alert("공방장님! 선생님께 아이템 결재가 성공적으로 올라갔습니다. 🛠️");
  };

  // 성찰 센터 멘트 로직
  const selectedRefStudentPraises = useMemo(() => {
    if (!refTarget) return [];
    return safeArray(db.approvedPraises).filter(p => p.toId == refTarget);
  ...
  const handleAddStudent = () => {
    if(!newStudentName) return alert("이름을 입력하세요.");
    const nextId = safeStudents.length > 0 ? Math.max(...safeStudents.map(s=>s.id)) + 1 : 1;
    sync({ students: [...safeStudents, { id: nextId, name: newStudentName, role: newStudentRole || '역할 없음', group: parseInt(newStudentGroup), isLeader: false }] });
    setNewStudentName(""); setNewStudentRole(""); alert("학생 전입 완료!");
  };
  const handleRemoveStudent = (id) => { if(window.confirm("이 학생을 삭제할까요?")) sync({ students: safeStudents.filter(s => s.id !== id) }); };
  const partialReset = (type) => {
    if(window.confirm(`정말 해당 데이터를 초기화할까요?`)) {
      if(type === 'donations') sync({ donations: [] }); if(type === 'status') sync({ studentStatus: {}, penaltyCount: {} }); if(type === 'exp') sync({ roleExp: {} }); alert("초기화 완료!");
    }
  };
  const closeSemester = () => {
    if(window.prompt("1학기를 마감하시겠습니까? 누적 데이터를 제외한 점수가 리셋됩니다. '마감'을 입력하세요.") === "마감") {
      sync({ roleExp: {}, usedCoins: {}, penaltyCount: {}, studentStatus: {}, pendingReflections: [], pendingPraises: [], donations: [] }); alert("학기 마감 완료! 🌱");
    }
  };
  const factoryReset = () => {
    if(window.prompt("초기화하시겠습니까? '초기화'를 입력하세요") === "초기화") {
      sync({ roleExp: {}, usedCoins: {}, penaltyCount: {}, studentStatus: {}, pendingReflections: [], pendingPraises: [], approvedPraises: [], donations: [], pendingShopItems: [], funding: [{ id: 1, name: "체육 시간 추가", target: 1000, current: 0 }, { id: 2, name: "팝콘 파티", target: 2000, current: 0 }], manualRepOffset: 0, shieldPoints: 100, allTime: { exp: {}, penalty: {}, donate: {}, fund: {} } }); alert("전체 리셋 완료.");
    }
  };

  if (isLoading) return <div className="min-h-screen bg-amber-50 flex items-center justify-center text-amber-600 font-black text-2xl">달보드레 교실 문 여는 중... 🌸</div>;

  return (
    <div className="min-h-screen bg-amber-50/50 pb-32 font-sans text-slate-800 transition-all">
      
      {/* 1. 아기자기한 명성 전광판 (파스텔 톤) */}
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8 relative z-10">
          <div className="text-center md:text-left flex-1">
            ...
            <div className="flex items-center justify-center md:justify-start gap-4">
              ...
              <div className="w-6 h-6 rounded-full bg-amber-400 animate-breathe shadow-[0_0_20px_rgba(251,191,36,0.8)] mt-6"></div>
            </div>
          ...
          <div className="flex flex-col items-center bg-white/80 p-8 rounded-[40px] border-4 border-white shadow-xl backdrop-blur-md">
            ...
            <span className="text-xs font-black text-emerald-600 uppercase tracking-widest bg-emerald-50 px-3 py-1 rounded-full mb-2">보호막 에너지</span>
      ...
      <div className="bg-pink-100 text-pink-700 py-3 overflow-hidden shadow-sm border-b border-pink-200 relative z-20">
        <div className="whitespace-nowrap animate-[shimmer_30s_linear_infinite] flex gap-20 text-sm font-black">
          <span className="flex gap-4 items-center"><MessageCircleHeart className="w-5 h-5 text-pink-500"/> 온기 우체통: {safeArray(db.approvedPraises).map(p => `[${SEL_OPTIONS.find(opt=>opt.name===p.tag)?.short||'칭찬'}] ${allStats.find(s=>s.id==p.toId)?.name||'나 자신'}: "${p.text}"`).join(' 🌸 ') || '서로에게 따뜻한 마음을 전해볼까요?'}</span>
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8">
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-12 animate-in fade-in duration-500">
                    ...
                    <div className="bg-purple-900/50 p-6 rounded-3xl border border-purple-400/50">
                      ...
                      <div className="space-y-3">
                        {safeArray(db.blackMarketItems).map(b => (
                          <div key={b.id} className="flex justify-between items-center bg-black/40 p-3 rounded-xl"><span className="text-sm font-bold text-purple-200">{b.name}</span>
            ...
            <div className="flex justify-between items-end border-b-2 border-amber-200 pb-4 mb-6 mt-8">
              <div>
              ...
              <button onClick={() => setShowPraiseModal(true)} className="bg-pink-400 text-white px-8 py-4 rounded-full font-black shadow-lg hover:bg-pink-500 active:scale-95 transition-all flex items-center gap-2"><Heart className="w-6 h-6 fill-white"/> 온기 우체통</button>
            </div>
              ...
              {sortedDashboardStats.map(s => (
                <div key={s.id} className={`p-5 rounded-[30px] border-4 shadow-sm transition-all relative flex flex-col bg-white hover:shadow-xl ${s.status === 'crisis' ? 'border-red-300 bg-red-50' : (s.status === 'pending' ? 'border-orange-300 bg-orange-50' : 'border-white hover:border-amber-200')}`}>
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <div className="text-right"><p className="text-[9px] font-black text-slate-400 uppercase">My Coins</p><p className="font-black text-amber-600 text-xl">{s.coins} 🪙</p></div>
                  </div>
                  
                  <div className="flex items-center justify-between mb-3 border-b border-slate-100 pb-3">
                    <div>
                      <p className="text-[10px] font-black text-slate-400">{s.group}모둠 · {s.role}</p>
                      <h3 className="text-xl font-black flex items-center gap-1 text-slate-800">{s.name} {s.isLeader && <Crown className="w-4 h-4 text-amber-400 fill-amber-400"/>}</h3>
                    </div>
                    {/* 🔥 뱃지 소형화하여 우측 배치 */}
                    <div className={`text-[10px] font-black px-2 py-1 rounded-lg border ${s.mastery.bg} ${s.mastery.color} ${s.exp >= 20 ? 'border-amber-300 shadow-sm' : 'border-transparent'} text-center leading-tight`}>
                ...
                <div className="text-left space-y-8 bg-emerald-50/50 p-10 rounded-[40px] border-2 border-emerald-100 shadow-inner relative z-10">
                  ...
                  {refTarget && (
                    <div className="bg-pink-50 border-2 border-pink-200 p-6 rounded-3xl text-pink-800 animate-in fade-in slide-in-from-top-4">
                      ...
                      {randomPraise ? (
                        <p className="text-sm font-bold leading-relaxed bg-white p-4 rounded-2xl shadow-sm">"예전에 친구가 <b>{randomPraise.tag}</b> 역량으로 널 칭찬했었어! 👉 <b>{randomPraise.text}</b> 넌 우리 반의 보물이야."</p>
                      ) : (
            ...
            <section className="flex-1 p-10 overflow-y-auto bg-slate-50/30">
              ...
              {helpSubTab === 'magistrate' && (
                <div className="space-y-6 animate-in fade-in">
                  <h3 className="text-2xl font-black text-slate-800 mb-8 flex items-center gap-2 bg-blue-100 inline-block px-5 py-2 rounded-full"><BookOpen className="text-blue-600"/> 현령 향리 관리소</h3>
                          ...
                          {groupMembers.map(s => (
                            <div key={s.id} className="bg-slate-50 p-5 rounded-3xl border border-slate-100 flex items-center justify-between shadow-sm">
                              ...
                              <div className="flex items-center gap-2 bg-white p-2 rounded-2xl border border-slate-100 shadow-sm">
                                ...
                                <span className="w-16 text-center font-black text-blue-600 text-2xl">{s.exp}<span className="text-sm ml-1 text-slate-400">회</span></span>
              ...
              {helpSubTab === 'shop' && (
                <div className="space-y-8 animate-in fade-in">
                  ...
                  <div className="bg-amber-50 p-8 rounded-[40px] border-4 border-amber-200 shadow-sm mb-10 relative overflow-hidden">
                    ...
                    <h4 className="text-2xl font-black text-amber-900 mb-2 relative z-10 flex items-center gap-2"><Hammer className="w-6 h-6"/> 장인의 공방 (특산품 기획소)</h4>
                    ...
                    {safeArray(db.shopItems).map(item => (
                      <div key={item.id} className="bg-white p-8 rounded-[40px] shadow-sm border-2 border-slate-100 flex flex-col justify-between hover:border-amber-200 transition-colors">
                         ...
                         <div className="flex gap-4">
                           <select id={`buyer_${item.id}`} className="flex-1 p-5 rounded-2xl bg-slate-50 border-none font-bold outline-none text-base shadow-inner focus:ring-2 ring-amber-100"><option value="">누가 살까요?</option>{allStats.map(s => <option key={s.id} value={s.id}>{s.name} ({s.coins}🪙)</option>)}</select>
                           <button onClick={() => {
                             ...
                             if(window.confirm(`${user.name} 코인 차감할까요?`)) { sync({ usedCoins: { ...db.usedCoins, [sid]: (db.usedCoins[sid] || 0) + item.price } }); alert("결제 성공! 🎁"); playSound('buy'); }
        ...
        {activeTab === 'admin' && isAuthenticated === 'teacher' && (
          <div className="bg-white rounded-[50px] shadow-sm border border-slate-100 flex flex-col lg:flex-row overflow-hidden min-h-[750px] animate-in slide-in-from-right duration-300">
             <aside className="w-full lg:w-72 bg-slate-900 p-10 flex flex-col gap-3 shrink-0 border-r border-slate-800">
                ...
                <button onClick={() => setAdminSubTab('shopAdmin')} className={`w-full p-4 rounded-2xl font-black text-left flex items-center gap-4 transition-all text-base ${adminSubTab === 'shopAdmin' ? 'bg-blue-600 text-white shadow-lg translate-x-2' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}><ShoppingCart className="w-5 h-5"/> 상점 통제소</button>
                ...
                <button onClick={() => setAdminSubTab('settings')} className={`w-full p-4 rounded-2xl font-black text-left flex items-center gap-4 transition-all text-base ${adminSubTab === 'settings' ? 'bg-blue-600 text-white shadow-lg translate-x-2' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}><Settings className="w-5 h-5"/> 환경 설정</button>
                  ...
                  <div className="space-y-8 animate-in fade-in">
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                       <div className="bg-white p-10 rounded-[40px] shadow-sm border border-blue-100 flex flex-col items-center text-center relative overflow-hidden">
                          ...
                          <p className="text-slate-500 font-bold mb-8 text-sm leading-relaxed">미션을 발동하면 학생들 카드에 완료 버튼이 생깁니다.<br/>학생이 누를 때마다 즉시 0.5점씩 오르며, 전원 성공 시 엄청난 보너스가 터집니다!</p>
                          ...
                          <div className="w-full space-y-4 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                            ...
                            {safeArray(db.pendingShopItems).map(item => (
                              <div key={item.id} className="bg-amber-50 p-5 rounded-2xl border border-amber-200 text-left shadow-sm">
                                ...
                                <div className="flex gap-2">
                                  <button onClick={() => {
                                    const next = db.pendingShopItems.filter(i => i.id !== item.id);
                                    sync({ pendingShopItems: next, shopItems: [item, ...safeArray(db.shopItems)] });
                                    alert("상점 등록 완료!"); playSound('good');
                                  }} className="flex-1 bg-amber-500 text-white py-3 rounded-xl font-black text-sm hover:bg-amber-600 shadow-md">상점에 출시 허가</button>
                                  <button onClick={() => {
                                    const next = db.pendingShopItems.filter(i => i.id !== item.id); sync({ pendingShopItems: next }); alert("반려되었습니다.");
                            ...
                            {safeArray(db.pendingPraises).map(p => (
                              <div key={p.id} className="bg-pink-50 p-5 rounded-2xl border border-pink-200 text-left shadow-sm">
                                ...
                                <button onClick={() => {
                                  ...
                                  if(p.toId !== 'me') { 
                                    sync({ pendingPraises: next, approvedPraises: app, roleExp: { ...db.roleExp, [p.toId]: (db.roleExp[p.toId]||0) + 1 }, allTime: { ...db.allTime, exp: { ...db.allTime.exp, [p.toId]: (db.allTime.exp?.[p.toId]||0)+1 } } }); 
                  ...
                  <div className="space-y-8 animate-in fade-in">
                    ...
                    <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 space-y-8">
                      <div className="flex flex-wrap gap-4">
                         <button onClick={() => sync({ settings: { ...db.settings, forceShopOpen: !db.settings.forceShopOpen } })} className={`flex-1 py-5 rounded-2xl font-black text-lg transition-all ${db.settings.forceShopOpen ? 'bg-amber-500 text-white shadow-xl' : 'bg-slate-100 text-slate-500'}`}>정규 상점 개방: {db.settings.forceShopOpen ? 'ON' : 'OFF'}</button>
                      ...
                      <div className="pt-8 border-t border-slate-200">
                        <h4 className="font-black text-lg text-slate-700 mb-4">물품 직접 등록</h4>
                  ...
                  <div className="space-y-8 animate-in fade-in">
                    ...
                    <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 space-y-8">
                       ...
                       <div className="pt-6 border-t border-slate-100 flex flex-col md:flex-row gap-8 items-end">
                          <div>
                            <label className="block text-sm font-black text-slate-500 mb-4">도움실 접속 비밀번호</label>
                  ...
                  <div className="animate-in fade-in space-y-8">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                       <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm text-center">
                         ...
                         <p className="text-xs text-slate-500 font-bold mb-6">전광판의 기부 내역만 삭제합니다.</p>
                       ...
                       <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm text-center">
                         ...
                         <p className="text-xs text-slate-500 font-bold mb-6">모든 위기/대기 학생을 복구합니다.</p>
                       ...
                       <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm text-center">
                         ...
                         <p className="text-xs text-slate-500 font-bold mb-6">1인 1역 숙련도(횟수)를 0으로 리셋.</p>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       <div className="bg-blue-50 border-4 border-blue-200 p-10 rounded-[40px] text-center shadow-lg">
                          ...
                          <p className="text-blue-600 font-bold mb-8 text-sm">학생들의 단기 코인과 숙련도는 0이 되지만,<br/><b>1년 누적 장기 데이터는 보존</b>됩니다.</p>
                       ...
                       <div className="bg-red-50 border-4 border-red-200 p-12 rounded-[50px] text-center shadow-lg">
                          ...
                          <p className="text-red-600 font-bold mb-10 text-lg">초기화 시 <span className="underline">학생 명단을 제외한</span> 모든 데이터가 '0'이 됩니다.</p>
      ...
      {showModal === 'password' && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-6 z-[9999]">
          <div className="bg-white rounded-[60px] p-16 w-full max-w-lg text-center shadow-2xl animate-in zoom-in-95 border-4 border-blue-100">
            ...
            <p className="text-center text-slate-400 font-bold mb-10 text-sm">교사 권한(6505) 또는 도움실 비밀번호가 필요해요.</p>
            <input type="password" value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key === 'Enter' && handleLogin()} className="w-full text-center text-6xl tracking-[15px] font-black p-8 border-4 border-blue-100 rounded-[40px] outline-none mb-12 bg-blue-50 focus:border-blue-400 focus:bg-white shadow-inner" autoFocus />
            <div className="flex gap-4">
              ...
              <button onClick={handleLogin} className="flex-1 py-6 rounded-[30px] font-black bg-blue-500 text-white text-xl shadow-xl hover:bg-blue-600 active:scale-95 transition-all">접속하기</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showPraiseModal && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center z-[1000] p-4">
          <div className="bg-white p-10 rounded-[50px] w-full max-w-md shadow-2xl animate-in zoom-in-95 border-4 border-pink-200">
            ...
            <div className="space-y-6 mb-10">
              <select value={praiseTarget} onChange={(e)=>setPraiseTarget(e.target.value)} className="w-full p-5 rounded-3xl bg-pink-50 border-2 border-pink-100 font-black text-lg outline-none focus:border-pink-400 focus:bg-white shadow-inner text-pink-900">
              ...
              <select value={praiseTag} onChange={(e)=>setPraiseTag(e.target.value)} className="w-full p-5 rounded-3xl bg-pink-50 border-2 border-pink-100 font-black text-lg outline-none focus:border-pink-400 focus:bg-white shadow-inner text-pink-900">
                <option value="">어떤 역량인가요?</option>
                {SEL_OPTIONS.map(opt => <option key={opt.name} value={opt.name}>{opt.name}</option>)}
              </select>
              <textarea value={praiseText} onChange={(e)=>setPraiseText(e.target.value)} rows="4" placeholder="어떤 구체적인 행동을 했는지 적어주세요! (선생님 승인 시 전광판에 등록됩니다)" className="w-full p-5 rounded-3xl bg-pink-50 border-2 border-pink-100 font-black outline-none focus:border-pink-400 focus:bg-white resize-none shadow-inner placeholder:font-bold text-pink-900"/>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setShowPraiseModal(false)} className="flex-1 py-5 bg-slate-100 text-slate-500 rounded-[25px] font-black text-lg hover:bg-slate-200 transition-colors">취소</button>
              <button onClick={submitPraise} className="flex-1 py-5 bg-pink-500 text-white rounded-[25px] font-black text-lg shadow-xl hover:bg-pink-600 active:scale-95 transition-all">보내기</button>
            </div>
          </div>
        </div>
      )}
