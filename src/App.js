...
const INITIAL_DB = {
  ...
  studentStatus:{}, pendingReflections:[], pendingPraises:[],
        ...
        <div className="max-w-[1400px] w-full mx-auto relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-end">
          ...
          <div className="lg:col-span-5 h-full">
            <TimerWidget
              ...
              onStopwatch={startStopwatch} onCountdown={startCountdown}
              onPause={pauseTimer} onResume={resumeTimer} onReset={resetTimer} onBreak={startBreak}
      ...
      <div className="bg-pink-100 text-pink-700 py-3 overflow-hidden shadow-sm border-b-2 border-pink-200 relative z-20">
        <div className="whitespace-nowrap animate-[shimmer_35s_linear_infinite] flex gap-20 text-sm font-black">
          <span className="flex gap-4 items-center">
            ...
            온기 우체통: {safeArray(db.approvedPraises).map(p=>`[${SEL_OPTIONS.find(o=>o.name===p.tag)?.short||'칭찬'}] ${allStats.find(s=>s.id==p.toId)?.name||'나'}: "${p.text}"`).join(' 💌 ')||'따뜻한 마음을 전해볼까요?'}
          </span>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-white p-8 rounded-[40px] border-2 border-blue-100 shadow-sm flex flex-col justify-between">
                ...
                <div className="flex items-center justify-between bg-yellow-50 p-5 rounded-2xl border border-yellow-200">
                  <div className="flex flex-col gap-2 flex-1 pr-4">
                    ...
                    <div className="flex gap-1.5 max-w-[150px]">
                      {[1,2,3,4,5].map(step=>(
                        <div key={step} className={`h-2.5 flex-1 rounded-full transition-all duration-300 ${step<=(db.coopQuest?.goodWeek||0)?'bg-yellow-400':'bg-yellow-200/50'}`}/>
                      ))}
                    </div>
                  </div>
                ...
                {(db.coopQuest?.goodWeek||0)>=5 && (
                  <button onClick={completeGoodWeek} className="mt-4 w-full bg-yellow-400 text-yellow-900 font-black py-5 rounded-2xl text-lg animate-pulse hover:bg-yellow-500">🌟 최종 승인 및 명성 획득!</button>
                )}
              </div>
              ...
              <div className={`p-8 rounded-[40px] border-2 flex flex-col items-center justify-center min-h-[200px] ${db.timeAttack?.isActive?'bg-red-50 border-red-300':'bg-slate-50 border-dashed border-slate-200'}`}>
                {db.timeAttack?.isActive ? (
                  <>
                    <div className="flex items-center gap-3 mb-4"><Timer className="w-8 h-8 text-red-500 animate-pulse"/><h2 className="text-lg font-black text-red-600">돌발 타임어택 진행 중!</h2></div>
                    ...
                    <p className="text-sm font-bold text-red-400 mt-5">성공 시 학급 명성 +{db.timeAttack.rewardRep}점</p>
                  </>
                ) : (
                  ...
                  <div key={s.id} className={`p-6 rounded-[40px] border-4 shadow-sm transition-all relative flex flex-col bg-white hover:shadow-xl hover:-translate-y-1 ${s.status==='crisis'?'border-slate-300 bg-slate-100 opacity-60 grayscale':s.status==='pending'?'border-orange-300 bg-orange-50':'border-white hover:border-amber-300'}`}>
                    ...
                    {db.timeAttack?.isActive && s.status!=='crisis' && (
                      <div className="absolute -top-4 -right-4 z-20">
                        <button onClick={()=>toggleTimeAttackClear(s.id)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full font-black text-xs shadow-md border-2 ${isTaCleared?'bg-green-500 text-white border-green-600':'bg-white text-slate-400 border-slate-200 hover:text-red-500'}`}>
                          {isTaCleared?<><CheckCircle2 className="w-3 h-3"/>완료</>:<><Timer className="w-3 h-3"/>도전 중</>}
                        </button>
                      </div>
                    )}
                    ...
                    {isAnim && (
                      <div className="absolute top-6 left-1/2 -translate-x-1/2 z-30 pointer-events-none">
                        <span className="text-4xl font-black text-amber-500 drop-shadow-xl animate-flyToScore">+1</span>
                      </div>
                    )}
                    ...
                    <div className="flex justify-between items-center mb-5 border-b-2 border-slate-100/50 pb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">My Coins</p>
                          <p className={`font-black text-2xl leading-none ${s.status==='crisis'?'text-slate-500':'text-amber-600'}`}>{s.coins}<span className="text-base ml-1">🪙</span></p>
                        </div>
                      </div>
                      {isAuthenticated==='teacher' && (
                        <button onClick={()=>openNoteModal(s.id)} className="p-3 rounded-2xl bg-yellow-50 text-yellow-600 hover:bg-yellow-100 border border-yellow-200 shadow-sm">
                    ...
                    <div className="flex flex-col mb-5 gap-3">
                      <p className="text-[12px] font-bold text-slate-400 tracking-wide truncate bg-slate-50 w-max px-3 py-1 rounded-md">
                        {s.group}모둠 · {s.role}
                      </p>
                      ...
                      <div className={`text-sm font-black px-3 py-1.5 rounded-2xl border-2 self-start shadow-sm ${s.status==='crisis'?'bg-slate-200 border-slate-300 text-slate-500':`${s.mastery.bg} ${s.mastery.color}`}`}>
                        {s.mastery.label} <span className="opacity-70">({s.exp})</span>
                      </div>
                    </div>
                    ...
                    <div className="mt-auto pt-2">
                      {s.status==='normal' && (
                        <button onClick={()=>handleGivePenalty(s.id)} className="w-full py-3 bg-slate-50 border border-slate-200 hover:bg-red-50 text-slate-400 hover:text-red-600 hover:border-red-200 rounded-[18px] font-black text-sm flex items-center justify-center gap-2 transition-all">
                          <AlertTriangle className="w-4 h-4"/> 위기 상태로 지정
                        </button>
                      )}
          ...
          <div className="bg-white rounded-[50px] shadow-sm border-4 border-indigo-50 flex flex-col lg:flex-row min-h-[800px] animate-in fade-in overflow-hidden">
            ...
            <aside className="w-full lg:w-80 bg-indigo-50/50 p-10 border-r-2 border-white flex flex-col gap-5 shrink-0">
              ...
              {[
                ...
                <button key={m.key} onClick={()=>setHelpSubTab(m.key)} className={`w-full p-6 rounded-3xl font-black text-left flex items-center gap-4 text-lg transition-all ${helpSubTab===m.key?`${m.active} text-white shadow-xl translate-x-2`:'bg-white text-indigo-400 hover:translate-x-1'}`}>
                  {m.icon} {m.label}
                </button>
              ))}
            </aside>
                ...
                <div className="space-y-8 animate-in fade-in">
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100">
                    ...
                    <div className="flex gap-3 mb-6 bg-indigo-50 p-6 rounded-3xl border border-indigo-100">
                      ...
                      <button onClick={()=>{
                        const val=document.getElementById('new_role_input_hr').value.trim();
                        ...
                        document.getElementById('new_role_input_hr').value="";
                      }} className="bg-indigo-600 text-white px-8 rounded-xl font-black text-lg shadow-md hover:bg-indigo-700 active:scale-95 transition-transform">목록에 추가</button>
                    </div>
                      ...
                      <div key={f.id} className="bg-gradient-to-br from-blue-500 to-indigo-600 p-10 rounded-[40px] shadow-xl text-white flex flex-col justify-between relative overflow-hidden border-4 border-blue-400">
                        ...
                        <div className="relative z-10">
                          ...
                          <div className="flex justify-between items-end text-xl font-black mb-4">
                            <span>현재: {toInt(f.current)}p</span><span className="text-blue-200">목표: {toInt(f.target,1)}p</span>
                          </div>
                          <div className="w-full h-6 bg-black/30 rounded-full mb-10 overflow-hidden border border-white/20">
                            <div className="h-full bg-yellow-400 transition-all duration-1000" style={{ width:`${Math.min((toInt(f.current)/toInt(f.target,1))*100,100)}%` }}/>
                          </div>
                        </div>
                        ...
                        {safeArray(db.pendingReflections).map(r=>{
                          ...
                          return (
                            <div key={r.id} className="bg-white p-6 rounded-3xl border border-emerald-100 flex justify-between items-start gap-4 shadow-sm">
                              <div>
                                <p className="text-sm font-black text-emerald-600 mb-1">{s?.name} · {SEL_OPTIONS.find(o=>o.name===r.tag)?.short} · {r.date}</p>
                                <p className="font-bold text-slate-700 whitespace-pre-wrap">{r.text}</p>
                              </div>
                              <div className="flex gap-2 shrink-0">
                                ...
                                <button onClick={()=>sync({ pendingReflections:safeArray(db.pendingReflections).filter(x=>x.id!==r.id) })} className="bg-slate-200 text-slate-600 px-5 py-3 rounded-2xl font-black text-sm hover:bg-slate-300">반려</button>
                              </div>
                            </div>
                          );
                        })}
                      ...
                      <div className="space-y-4">
                        {safeArray(db.pendingShopItems).map(item=>(
                          <div key={item.id} className="bg-white p-6 rounded-3xl border border-amber-100 flex justify-between items-center gap-4 shadow-sm">
                            <div>
                              <p className="text-sm font-black text-amber-600 mb-1">{item.creator} 장인 제작</p>
                              <p className="font-black text-xl text-slate-800">{item.name} <span className="text-amber-500">{item.price}🪙</span></p>
                            </div>
                            <div className="flex gap-2 shrink-0">
                              <button onClick={()=>{ sync({ shopItems:[...safeArray(db.shopItems),item], pendingShopItems:safeArray(db.pendingShopItems).filter(x=>x.id!==item.id) }); alert(`"${item.name}" 상점 등록 완료!`); playSound('good'); }} className="bg-amber-500 text-white px-5 py-3 rounded-2xl font-black text-sm hover:bg-amber-600 shadow-md">승인</button>
                              <button onClick={()=>sync({ pendingShopItems:safeArray(db.pendingShopItems).filter(x=>x.id!==item.id) })} className="bg-slate-200 text-slate-600 px-5 py-3 rounded-2xl font-black text-sm hover:bg-slate-300">반려</button>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                      {[
                        ...
                        <div key={q.key} className={`${q.bgClass} p-4 rounded-2xl border flex gap-3 shadow-sm`}>
                          <input type="text" value={db.coopQuest?.[`${q.key}Name`]||q.defName} onChange={e=>sync({ coopQuest:{...db.coopQuest,[`${q.key}Name`]:e.target.value} })} onFocus={lockEditing} onBlur={unlockEditing} className="flex-1 p-3 rounded-xl text-sm font-bold border-none outline-none shadow-sm"/>
                          <input type="number" value={db.coopQuest?.[q.key]??q.defVal} onChange={e=>sync({ coopQuest:{...db.coopQuest,[q.key]:toInt(e.target.value)} })} onFocus={lockEditing} onBlur={unlockEditing} className={`w-20 p-3 rounded-xl text-base font-black ${q.textClass} border-none outline-none text-center shadow-sm`}/>
                        </div>
                      ))}
                    </div>
                      ...
                      {db.timeAttack?.isActive ? (
                        ...
                        <div className="space-y-4">
                          <div className="bg-white p-6 rounded-2xl border border-red-200 space-y-5 shadow-sm">
                            ...
                            <div className="flex gap-4">
                              <div className="flex-1"><label className="block text-[11px] font-black text-red-400 mb-1 ml-1">제한 시간(분)</label><input type="number" value={taMins} onChange={e=>setTaMins(e.target.value)} onFocus={lockEditing} onBlur={unlockEditing} className="w-full p-4 rounded-xl border border-red-100 font-black text-center outline-none focus:border-red-400"/></div>
                              <div className="flex-1"><label className="block text-[11px] font-black text-red-400 mb-1 ml-1">성공 보상(p)</label><input type="number" value={taReward} onChange={e=>setTaReward(e.target.value)} onFocus={lockEditing} onBlur={unlockEditing} className="w-full p-4 rounded-xl border border-red-100 font-black text-center outline-none focus:border-red-400"/></div>
                            </div>
                          </div>
                ...
                <div className="space-y-8 animate-in fade-in">
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 space-y-8">
                    <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200 shadow-sm">
                      <button onClick={()=>sync({ settings:{...db.settings,forceShopOpen:!db.settings?.forceShopOpen} })} className={`w-full py-5 rounded-2xl font-black text-xl transition-all shadow-md ${db.settings?.forceShopOpen?'bg-amber-500 text-white':'bg-white text-slate-500 border-2 border-slate-300'}`}>
                    ...
                    <div className="pt-6">
                      ...
                      <div className="flex gap-3 mb-6">
                        <input id="new_item_name_input" type="text" placeholder="새로운 물품 이름" className="flex-1 p-4 rounded-xl border border-slate-300 font-bold outline-none"/>
                        <input id="new_item_price_input" type="number" placeholder="가격" className="w-32 p-4 rounded-xl border border-slate-300 font-bold outline-none text-center"/>
                        <button onClick={()=>{ const n=document.getElementById('new_item_name_input').value; const p=document.getElementById('new_item_price_input').value; if(!n||!p) return alert("입력 오류"); sync({ shopItems:[...safeArray(db.shopItems),{ id:Date.now(),name:n,price:toInt(p),creator:'선생님' }] }); document.getElementById('new_item_name_input').value=""; document.getElementById('new_item_price_input').value=""; }} className="bg-blue-600 text-white px-8 rounded-xl font-black shadow-md hover:bg-blue-700">물품 추가</button>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {safeArray(db.shopItems).filter(i=>i&&i.name).map(item=>(
                    ...
                    <div className="pt-8 border-t border-slate-200 mt-8">
                      ...
                      <div className="flex gap-3 mb-6 bg-blue-50 p-6 rounded-3xl border border-blue-100">
                        <input id="new_fund_name_input" type="text" placeholder="새로운 펀딩 목표" className="flex-1 p-4 rounded-xl border border-blue-200 font-bold outline-none focus:border-blue-400"/>
                        <input id="new_fund_target_input" type="number" placeholder="목표 점수" className="w-32 p-4 rounded-xl border border-blue-200 font-bold outline-none focus:border-blue-400 text-center"/>
                        <button onClick={()=>{ const n=document.getElementById('new_fund_name_input').value; const t=document.getElementById('new_fund_target_input').value; if(!n||!t||toInt(t)===0) return alert("올바른 값을 입력하세요."); sync({ funding:[...safeArray(db.funding),{ id:Date.now(),name:n,target:toInt(t),current:0 }] }); document.getElementById('new_fund_name_input').value=""; document.getElementById('new_fund_target_input').value=""; }} className="bg-blue-600 text-white px-8 rounded-xl font-black shadow-md hover:bg-blue-700">펀딩 개설</button>
                      </div>
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 flex flex-col md:flex-row gap-10">
                    <div className="w-full md:w-1/3 space-y-4">
                      ...
                      <div className="bg-indigo-50 p-6 rounded-3xl border border-indigo-100 space-y-3 shadow-sm">
                        ...
                        <button disabled={!selectedReportStudent} onClick={()=>exportStudent(selectedReportStudent)} className={`w-full p-4 rounded-xl font-black text-sm flex items-center justify-center gap-2 ${selectedReportStudent?'bg-indigo-500 text-white hover:bg-indigo-600 shadow-md':'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>
                              ...
                              {Object.keys(counts).map(tag=>(
                                <div key={tag} className="flex items-center gap-5">
                                  ...
                                  <div className="flex-1 h-8 bg-white rounded-full overflow-hidden border border-slate-200 shadow-inner">
                                    <div className="h-full bg-gradient-to-r from-blue-400 to-indigo-500 transition-all duration-1000" style={{ width:`${(counts[tag]/max)*100}%` }}/>
                                  </div>
                          ...
                          <div key={s.id} className={`p-5 rounded-2xl border-2 shadow-sm ${bonus?'bg-yellow-50 border-yellow-300':'bg-slate-50 border-slate-200'}`}>
                            <div className="flex justify-between items-center">
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                {bonus&&<span className="text-xs bg-yellow-400 text-yellow-900 px-3 py-1 rounded-full font-black shadow-sm animate-pulse">개근!</span>}
                              </div>
                            </div>
                          </div>
                        );
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 space-y-10">
                      ...
                      {[
                        ...
                        <div key={p.field}>
                          ...
                          <div className="flex gap-3">
                            ...
                            <button onClick={()=>{ if(!p.val) return alert('입력하세요.'); sync({ settings:{...db.settings,[p.field]:p.val} }); alert('변경 완료!'); p.setter(''); }} className="bg-blue-600 text-white px-6 rounded-2xl font-black text-lg shadow-md">변경</button>
                          </div>
                          <p className="text-sm font-bold text-slate-400 mt-3 bg-white inline-block px-3 py-1 rounded-lg border border-slate-200">현재: {String(p.current)}</p>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="pt-8 border-t-2 border-slate-100">
                      <button onClick={toggleCumulativeStats} className={`w-full py-5 rounded-2xl font-black text-xl transition-all shadow-md flex items-center justify-center gap-3 ${db.settings?.showCumulativeStats?'bg-blue-600 text-white':'bg-white text-slate-500 border-2 border-slate-300 hover:border-blue-300'}`}>
                        <Eye className="w-6 h-6"/> 누적 스탯 표시: {db.settings?.showCumulativeStats?'ON (공개 중)':'OFF (비공개)'}
                      </button>
                    </div>
                    {/* 개별 코인 강제 조정 */}
                    <div className="pt-8 border-t-2 border-slate-100 bg-amber-50/50 p-10 rounded-[40px] border border-amber-100 mt-8">
                      ...
                      <div className="flex flex-col md:flex-row gap-4 bg-white p-6 rounded-3xl shadow-sm border border-amber-200">
                        <select id="coin_adjust_student" className="flex-1 p-4 rounded-2xl border border-slate-200 font-black outline-none text-lg focus:border-amber-400">
                          ...
                          {allStats.map(s=><option key={s.id} value={s.id}>{s.name} (현재: {s.coins}🪙)</option>)}
                        </select>
                    ...
                    <div className="pt-8 border-t-2 border-slate-100 bg-indigo-50/50 p-10 rounded-[40px] border border-indigo-100">
                      ...
                      <div className="flex flex-col md:flex-row gap-4 mb-10 bg-white p-6 rounded-3xl shadow-sm border border-indigo-200">
                        <input type="number" value={manualScoreInput} onChange={e=>setManualScoreInput(e.target.value)} onFocus={lockEditing} onBlur={unlockEditing} placeholder="강제 추가/차감 (예: 50 또는 -20)" className="flex-1 p-4 rounded-2xl border border-slate-200 font-black outline-none text-lg focus:border-indigo-400"/>
                        <button onClick={()=>{ const v=toInt(manualScoreInput); if(!v) return; if(window.confirm(`${v>0?'+'+v:v}점의 명성을 즉시 적용할까요?`)){ sync({ manualRepOffset:(db.manualRepOffset||0)+v }); setManualScoreInput(""); playSound('good'); } }} className="bg-indigo-600 text-white px-10 rounded-2xl font-black text-lg shadow-md hover:bg-indigo-700 active:scale-95 transition-transform">명성 적용</button>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {[
                          ...
                          <div key={f.key} className="bg-white p-5 rounded-2xl shadow-sm">
                            ...
                            <input type="number" value={f.section?(db.settings?.[f.section]?.[f.key]??f.def):(db.settings?.[f.key]??f.def)} onChange={e=>{ const v=toInt(e.target.value,f.def); const next=f.section?{ ...db.settings,[f.section]:{ ...db.settings[f.section],[f.key]:v } }:{ ...db.settings,[f.key]:v }; sync({ settings:next }); }} onFocus={lockEditing} onBlur={unlockEditing} className={`w-full p-4 rounded-xl border border-${f.color}-200 font-black text-xl text-${f.color}-600 outline-none focus:border-${f.color}-400`}/>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div className="animate-in fade-in space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="bg-blue-50 border-4 border-blue-200 p-10 rounded-[40px] text-center shadow-lg flex flex-col justify-between">
                      <div><History className="w-16 h-16 text-blue-500 mx-auto mb-5"/><h3 className="text-3xl font-black mb-4 text-blue-800">1학기 최종 마감</h3><p className="font-bold text-blue-600 mb-8 text-base">코인, 직업 점수, 출석, 성찰 등을 리셋하지만, 평생 기록(누적 데이터)은 보존합니다.</p></div>
                      <button onClick={closeSemester} className="w-full bg-blue-600 text-white py-5 rounded-[24px] font-black text-xl shadow-xl hover:bg-blue-700 active:scale-95 transition-all">학기 마감 실행</button>
                    </div>
                    <div className="bg-emerald-50 border-4 border-emerald-200 p-10 rounded-[40px] text-center shadow-lg flex flex-col justify-between">
                      <div><Briefcase className="w-16 h-16 text-emerald-500 mx-auto mb-5"/><h3 className="text-3xl font-black mb-4 text-emerald-800">직업 점수만 초기화</h3><p className="font-bold text-emerald-600 mb-8 text-base">이 주의 직업 활동이 끝났을 때, 학생들의 '현재 직업 숙련도 점수'만 0으로 리셋합니다.</p></div>
                      <button onClick={()=>{ if(window.prompt("직업 숙련도를 0으로 초기화하려면 '직업초기화'를 입력하세요.")==="직업초기화"){ sync({ roleExp:{} }); alert("직업 점수 초기화 완료."); } }} className="w-full bg-emerald-600 text-white py-5 rounded-[24px] font-black text-xl shadow-xl hover:bg-emerald-700 active:scale-95 transition-all">직업 초기화 실행</button>
                    </div>
                    <div className="bg-red-50 border-4 border-red-200 p-10 rounded-[40px] text-center shadow-lg flex flex-col justify-between">
                      <div><Trash2 className="w-16 h-16 text-red-500 mx-auto mb-5"/><h3 className="text-3xl font-black mb-4 text-red-800">시스템 공장 초기화</h3><p className="font-bold text-red-600 mb-8 text-base">명단과 세팅을 제외한 '모든 데이터(누적 포함)'를 영구적으로 완전 삭제합니다.</p></div>
                      <button onClick={factoryReset} className="w-full bg-red-600 text-white py-5 rounded-[24px] font-black text-xl shadow-xl hover:bg-red-700 active:scale-95 transition-all">공장 초기화 실행</button>
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-[5001] p-4">
          <div className="bg-white p-12 rounded-[50px] w-full max-w-xl shadow-2xl animate-in zoom-in-95 border-4 border-pink-100 relative overflow-hidden">
            ...
            <div className="bg-pink-50 border border-pink-200 p-4 rounded-2xl mb-8 text-center relative z-10 shadow-sm">
              <p className="text-sm font-bold text-pink-700"><Sparkles className="w-4 h-4 inline-block mb-1 text-pink-500"/> 이 주의 집중 SEL 테마: <span className="font-black bg-white px-2 py-1 rounded-md text-pink-900">{db.settings?.weeklyTheme||"없음"}</span></p>
  ...
  return (
    <div className={`p-6 md:p-8 rounded-[40px] border-4 shadow-2xl backdrop-blur-md ${bgClass} ${flashClass} flex flex-col h-full min-h-[300px]`}>
          ...
          {status==='idle' ? (
            <>
              <div className="flex bg-slate-200/50 p-1 rounded-lg">
                {[1,3,5,10].map(m=>(
                  <button key={m} onClick={()=>onCountdown(m)} className="px-2.5 py-1.5 bg-white hover:bg-slate-100 rounded text-xs font-black shadow-sm mx-0.5 transition-colors">{m}</button>
                ))}
              </div>
              <button onClick={onStopwatch} className="px-3 py-1.5 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg text-xs font-black flex items-center gap-1 shadow-sm transition-colors">
              ...
              <div className="flex gap-1 ml-1 bg-emerald-50 p-1 rounded-lg border border-emerald-100">
                <input type="number" value={breakInput} onChange={e=>setBreakInput(e.target.value)} onFocus={lockEditing} onBlur={unlockEditing} className="w-10 px-1 py-1 text-xs font-black text-center text-emerald-900 bg-white rounded border border-emerald-200 outline-none focus:border-emerald-500"/>
                <button onClick={()=>onBreak(Math.max(1,toInt(breakInput,defaultBreakMin)))} className="px-3 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded text-xs font-black flex items-center gap-1 shadow-sm transition-colors">
            ...
            <div className="flex items-center gap-2">
              {isRunning ? (
                <button onClick={onPause} className="px-4 py-2 bg-yellow-500 text-white rounded-lg text-sm font-black flex items-center gap-1 shadow-sm hover:bg-yellow-600 transition-colors"><Pause className="w-4 h-4 fill-white"/> 일시정지</button>
              ) : (
                <button onClick={onResume} className="px-4 py-2 bg-green-500 text-white rounded-lg text-sm font-black flex items-center gap-1 shadow-sm hover:bg-green-600 transition-colors"><Play className="w-4 h-4 fill-white"/> 계속</button>
              )}
      ...
      <div className="flex-1 flex items-center justify-center py-4">
        <span className="text-[110px] md:text-[150px] lg:text-[170px] font-black tracking-tighter tabular-nums leading-none text-slate-800 drop-shadow-md">{display}</span>
      </div>
    </div>
  );
}
