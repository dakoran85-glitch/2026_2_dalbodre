                ...
                <div className="space-y-8 animate-in fade-in">
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 space-y-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-6">
                      ...
                      ].map(f=>( <div key={f.key} className="bg-white p-5 rounded-2xl shadow-sm"><label className={`block text-sm font-black text-${f.color}-600 mb-3`}>{f.label}</label><input type="number" value={f.section?(db.settings?.[f.section]?.[f.key]??f.def):(db.settings?.[f.key]??f.def)} onChange={e=>{ const v=toInt(e.target.value,f.def); const next=f.section?{ ...db.settings,[f.section]:{ ...db.settings[f.section],[f.key]:v } }:{ ...db.settings,[f.key]:v }; sync({ settings:next }); }} onFocus={lockEditing} onBlur={unlockEditing} className={`w-full p-4 rounded-xl border border-${f.color}-200 font-black text-xl text-${f.color}-600 outline-none focus:border-${f.color}-400`}/></div> ))}
                    </div></div>
                  </div>
                </div>
              )}
