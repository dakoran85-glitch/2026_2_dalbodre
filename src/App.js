...
const playSound = (type) => {
  try {
    ...
    switch (type) {
      case 'good':
        tone(600, 0.1); tone(1200, 0.2, 0.1);
        break;
      case 'bad':
        tone(300, 0.15, 0, 'sawtooth'); tone(100, 0.2, 0.15, 'sawtooth');
        break;
      case 'buy':
        tone(500, 0.15, 0, 'square'); tone(900, 0.15, 0.1, 'square');
        break;
      case 'jackpot':
        [440, 554.37, 659.25, 880].forEach((f,i) => tone(f, 0.2, i*0.1, 'triangle', 0.2));
        break;
      case 'chime': // 차임벨 (카운트다운 종료)
        [523.25, 659.25, 783.99, 1046.5].forEach((f,i) => tone(f, 0.6, i*0.12, 'sine', 0.18));
        break;
      case 'softChime': // 부드러운 차임 (3분/1분 경고)
...
const INITIAL_DB = {
  ...
  // 🆕 v10
  attendance: {},         // { "2026-W16": { "1": [0,1,2], "2": [0] } }
  attendanceBonus: {},    // { "2026-W16": { "1": true } } — 보너스 지급 여부
  streakWeeks: {},        // { "1": 4 } — 누적 개근 주수
  notes: {},              // { "1": [{ id, date, text }] }
  extraAttendDays: 0      // 교사 보정 (공휴일)
};
...
export default function App() {
  ...
  const [showNoteModal, setShowNoteModal] = useState(null); // 학생 ID
  const [noteText, setNoteText] = useState("");
  const [showNotesInReport, setShowNotesInReport] = useState(false);
  const [timeLeftString, setTimeLeftString] = useState("");
  const [attendAnim, setAttendAnim] = useState(null); // { id, timestamp }
  ...
  const [timerStatus, setTimerStatus] = useState('idle'); // idle | class_sw | class_cd | break
  const [breakInput, setBreakInput] = useState("10");
  const [breakWarningLevel, setBreakWarningLevel] = useState(0); // 0=safe, 1=3min, 2=1min, 3=30sec
  ...
  const lastNotifiedRef = useRef({}); // 타임워치 알림 중복 방지
  ...
  // ── 주간 자동 정리 (지난 주 출석 데이터 삭제 + streak 처리) ─
  useEffect(() => {
    if (isLoading) return;
  ...
  const groupedByGroupStats = useMemo(
    () => [...allStats].sort((a,b) => a.group - b.group || a.id - b.id), [allStats]);
  ...
  const randomPraise = selectedRefStudentPraises.length
    ? selectedRefStudentPraises[Math.floor(Math.random() * selectedRefStudentPraises.length)] : null;
  ...
  const toggleAttendance = (sId) => {
    ...
    const weekKey = getWeekKey();
    const weekData = db.attendance?.[weekKey] || {};
    const days = safeArray(weekData[sId]);
    const alreadyToday = days.includes(todayIdx);

    // 취소는 언제나 가능, 새 출석은 08:30까지만
    if (!alreadyToday && !isAttendanceOpen())
      return alert("출석 시간이 지났어요. (08:30 마감)");
    ...
    if (!alreadyToday) {
      ...
      if (totalDays >= 5 && !alreadyBonus) {
        ...
        updates.attendanceBonus = {
          ...db.attendanceBonus,
          [weekKey]: { ...(db.attendanceBonus?.[weekKey] || {}), [sId]: true }
        };
  ...
  const teacherAddHoliday = () => {
    ...
    const next = (db.extraAttendDays || 0) + 1;
    // 보정 후 개근 달성 자동 체크
    const weekKey = getWeekKey();
    const updates = { extraAttendDays: next };
    const weekData = db.attendance?.[weekKey] || {};
    let newBonus = { ...(db.attendanceBonus?.[weekKey] || {}) };
    let newStreaks = { ...(db.streakWeeks || {}) };
    let newCoins = { ...(db.bonusCoins || {}) };
    let bonusCount = 0;
    ...
    updates.attendanceBonus = { ...db.attendanceBonus, [weekKey]: newBonus };
  ...
  const submitNote = () => {
    ...
    const next = [...safeArray(db.notes?.[sId]), note];
    sync({ notes: { ...db.notes, [sId]: next } });
    setShowNoteModal(null); setNoteText("");
    alert("📝 누가기록 저장 완료!");
  };
  const deleteNote = (sId, noteId) => {
    if (!window.confirm("삭제할까요?")) return;
    const next = safeArray(db.notes?.[sId]).filter(n => n.id !== noteId);
    sync({ notes: { ...db.notes, [sId]: next } });
  };
  ...
  const buildStudentReport = (s) => {
    ...
    if (s.enneagram && ENNEAGRAM_DATA[s.enneagram])
      md += `- 에니어그램: ${ENNEAGRAM_DATA[s.enneagram].name}\n`;
    md += `- 누적 완수: ${s.atExp}회 / 기부: ${s.atDonate}🪙 / 펀딩: ${s.atFund}🪙 / 위기: ${s.atPen}회\n`;
    md += `- 이번 주 출석: ${s.weeklyCount}/5 · 누적 개근 주수: ${s.streak}주\n`;
    md += `- 현재 숙련도: ${s.mastery.label} (누적 ${s.exp})\n`;

    md += `\n### 받은 칭찬 (${praises.length}건)\n`;
    if (praises.length) praises.forEach(p => {
      md += `- [${p.date}] ${SEL_OPTIONS.find(o=>o.name===p.tag)?.short || '-'}: "${p.text}"\n`;
    }); else md += `- (없음)\n`;

    md += `\n### 제출한 성찰 (${reflections.length}건)\n`;
    if (reflections.length) reflections.forEach(r => {
      md += `- [${r.date}] ${SEL_OPTIONS.find(o=>o.name===r.tag)?.short || '-'}: "${r.text}"\n`;
    }); else md += `- (없음)\n`;
  ...
  const exportStudent = (sId) => {
    ...
    if (!s) return;
    const md = `# 달보드레 학생 리포트\n생성일: ${formatDate()}\n\n${buildStudentReport(s)}`;
    downloadOrCopy(md, `report_${s.name}_${formatDate()}.md`);
  };
  const exportAll = () => {
  ...
  const downloadOrCopy = (text, filename) => {
    ...
    try {
      ...
      alert(`📋 클립보드에 복사 + 📥 ${filename} 다운로드 완료!\nAI(ChatGPT/Claude 등)에 붙여넣어 보세요.`);
    } catch (e) { alert("다운로드 실패. 클립보드에는 복사되었습니다."); }
  };

  // ── 타임워치 ─────────────────────────────────────────
  const startStopwatch = () => sync({
    timer: { mode: 'class_sw', startedAt: Date.now(), endTime: null, duration: null, isRunning: true, pausedElapsed: 0, pausedRemaining: null }
  });
  const startCountdown = (minutes) => {
    const ms = minutes * 60 * 1000;
    sync({
      timer: { mode: 'class_cd', startedAt: null, endTime: Date.now() + ms, duration: ms, isRunning: true, pausedElapsed: null, pausedRemaining: ms }
    });
  };
  const pauseTimer = () => {
    const t = db.timer || {};
    ...
    if (t.mode === 'class_sw') {
      ...
      sync({ timer: { ...t, isRunning: false, pausedElapsed: elapsed } });
    } else if (t.mode === 'class_cd' || t.mode === 'break') {
      const remaining = t.endTime - Date.now();
      sync({ timer: { ...t, isRunning: false, pausedRemaining: Math.max(0, remaining) } });
    }
  };
  const resumeTimer = () => {
    const t = db.timer || {};
    if (t.isRunning) return;
    if (t.mode === 'class_sw') {
      sync({ timer: { ...t, isRunning: true, startedAt: Date.now() - (t.pausedElapsed || 0) } });
    } else if (t.mode === 'class_cd' || t.mode === 'break') {
      sync({ timer: { ...t, isRunning: true, endTime: Date.now() + (t.pausedRemaining || 0) } });
    }
  };
  const resetTimer = () => sync({
    timer: { mode: 'idle', startedAt: null, endTime: null, duration: null, isRunning: false, pausedElapsed: null, pausedRemaining: null }
  });
  ...
  // ── 기존 핸들러 (직업/위기/기부/펀딩/퀘스트/타임어택/제출) ──
  ...
  const handleStartTimeAttack = () => {
    const mins = toInt(taMins, 10), reward = toInt(taReward, 100);
    const title = taTitle.trim() || "미션";
    if (mins <= 0 || reward <= 0) return alert("값을 확인하세요.");
    if (!window.confirm(`${mins}분 / ${reward}p로 타임어택 시작?`)) return;
  ...
  const submitArtisanItem = () => {
    ...
    sync({ pendingShopItems: [{ id: Date.now(), name: artisanItemName, price: toInt(artisanItemPrice), creator: artisan.name }, ...safeArray(db.pendingShopItems)] });
  ...
  const approvePraise = (p) => {
    ...
    sync(updates);
    alert(`승인 완료! (+${earned}🪙)`); playSound('good');
  };
  ...
  const handleAddStudent = () => {
    ...
    sync({ students: [...safeStudents, { id: nextId, name: newStudentName, role: '향리', group: toInt(newStudentGroup, 1), isLeader: false, enneagram: newStudentEnneagram }] });
    setNewStudentName(""); alert("전입 완료!");
  };
  const handleRemoveStudent = (id) => {
    if (window.confirm("삭제할까요?")) sync({ students: safeStudents.filter(s => s.id !== id) });
  };
  const closeSemester = () => {
    if (window.prompt("'마감' 입력:") !== "마감") return;
    sync({ roleExp: {}, bonusCoins: {}, usedCoins: {}, penaltyCount: {}, studentStatus: {}, pendingReflections: [], pendingPraises: [], donations: [], attendance: {}, attendanceBonus: {}, streakWeeks: {}, notes: {} });
    alert("학기 마감 완료!");
  };
  const factoryReset = () => {
    if (window.prompt("'초기화' 입력:") !== "초기화") return;
    sync({
      roleExp: {}, bonusCoins: {}, usedCoins: {}, penaltyCount: {}, studentStatus: {},
      pendingReflections: [], pendingPraises: [], approvedPraises: [], donations: [],
  ...
  // 👉 다음 파트(3/3)에서 JSX return
  // ══════════════════════════════════════════════════════════
  // 🖼  RENDER
  // ══════════════════════════════════════════════════════════
  const todayIdx = getTodayWeekdayIdx();
  const attendOpen = isAttendanceOpen();
  ...
  return (
    <div className="min-h-screen bg-amber-50/50 pb-32 font-sans text-slate-800 transition-all">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-5 space-y-4">
              <div className="flex items-center gap-4 bg-white/60 p-4 rounded-3xl border-2 border-white shadow-sm backdrop-blur-sm">
                <ShieldCheck className={`w-14 h-14 ${shieldPoints > 0 ? 'text-blue-400 animate-pulse' : 'text-slate-300'}`} />
                <div className="flex-1">
                  <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest bg-blue-50 px-2 py-1 rounded-full">초월 보호막</span>
                  <p className={`text-4xl font-black ${shieldPoints > 0 ? 'text-blue-500' : 'text-slate-300'}`}>{shieldPoints}</p>
                </div>
              </div>
              ...
              <div className="w-full h-8 bg-white/50 rounded-full overflow-hidden shadow-inner border-4 border-amber-200 relative">
              ...
              <div className="flex justify-between items-center">
                <div className="flex-1 overflow-hidden whitespace-nowrap text-xs font-bold text-amber-700 bg-white/50 px-3 py-1 rounded-full border border-amber-200 mr-3">
                  <span className="animate-[shimmer_20s_linear_infinite] inline-block">
                    ✨ 기부: {safeArray(db.donations).map(d => `${d.name}(${d.amount}p)`).join(' · ') || '기다려요!'}
                  </span>
                </div>
                <span className="text-xs font-black text-orange-600 bg-white px-3 py-1 rounded-full shadow-sm border border-orange-200 shrink-0">목표: {db.settings?.targetScore || 5000}p</span>
              </div>
            </div>

            {/* 중앙: 세계수 */}
            <div className="lg:col-span-3 flex items-center justify-center py-4">
              <div className="scale-110">{renderEvolution(evolutionLevel)}</div>
            </div>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white p-6 rounded-[30px] border-2 border-blue-100 shadow-sm">
                <h3 className="text-[13px] font-black text-blue-600 mb-4 flex items-center gap-2"><Zap className="w-4 h-4"/> 학급 공동 퀘스트</h3>
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <button onClick={() => addCoopScore(db.coopQuest?.q1 || 50, db.coopQuest?.q1Name || "바른 생활")} className="bg-indigo-50 hover:bg-indigo-100 text-indigo-700 py-3 rounded-2xl font-black text-sm border border-indigo-200 active:scale-95 truncate px-2">{db.coopQuest?.q1Name || "바른 생활"} +{db.coopQuest?.q1 || 50}</button>
                  <button onClick={() => addCoopScore(db.coopQuest?.q2 || 20, db.coopQuest?.q2Name || "환대")} className="bg-pink-50 hover:bg-pink-100 text-pink-700 py-3 rounded-2xl font-black text-sm border border-pink-200 active:scale-95 truncate px-2">{db.coopQuest?.q2Name || "환대"} +{db.coopQuest?.q2 || 20}</button>
                  <button onClick={() => addCoopScore(db.coopQuest?.q3 || 20, db.coopQuest?.q3Name || "전담 우수")} className="bg-emerald-50 hover:bg-emerald-100 text-emerald-700 py-3 rounded-2xl font-black text-sm border border-emerald-200 active:scale-95 col-span-2 truncate px-2">{db.coopQuest?.q3Name || "전담 우수"} +{db.coopQuest?.q3 || 20}</button>
                </div>
                <div className="flex items-center justify-between bg-yellow-50 p-3 rounded-2xl border border-yellow-200">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <button onClick={() => adjustGoodWeek(-1)} className="p-2 bg-white rounded-lg text-red-500 border border-red-100"><Minus className="w-4 h-4"/></button>
                    <span className="font-black text-lg text-yellow-600 w-10 text-center">{db.coopQuest?.goodWeek || 0}/5</span>
                    <button onClick={() => adjustGoodWeek(1)} className="p-2 bg-white rounded-lg text-green-500 border border-green-100"><Plus className="w-4 h-4"/></button>
                  </div>
                </div>
              ...
              <div className={`p-6 rounded-[30px] border-2 flex flex-col items-center justify-center min-h-[180px] ${db.timeAttack?.isActive ? 'bg-red-50 border-red-300' : 'bg-slate-50 border-dashed border-slate-200'}`}>
                {db.timeAttack?.isActive ? (
                  <>
                    ...
                    <p className="text-xl font-black text-slate-800 mb-4 text-center">{db.timeAttack.title}</p>
                    ...
                    <p className="text-xs font-bold text-red-400 mt-3">성공시 +{db.timeAttack.rewardRep}점</p>
                  </>
                ) : (
                  <><Timer className="w-10 h-10 mb-3 opacity-30 text-slate-400"/><p className="font-black text-sm text-slate-400 opacity-70">발동된 타임어택 없음</p></>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { title: '🏆 역할 완수 TOP5', data: topExp, unit: '회', color: 'blue', icon: <CheckCircle2 className="w-5 h-5"/>, key: 'atExp' },
                { title: '🏆 기부 천사 TOP5', data: topDonate, unit: '🪙', color: 'amber', icon: <Coins className="w-5 h-5"/>, key: 'atDonate' },
                { title: '🏆 펀딩 기여 TOP5', data: topFund, unit: '🪙', color: 'pink', icon: <Target className="w-5 h-5"/>, key: 'atFund' }
              ].map(c => (
                <div key={c.title} className={`bg-gradient-to-br from-${c.color}-50 to-${c.color}-100 p-6 rounded-[32px] shadow-sm border border-${c.color}-200`}>
                  <h4 className={`text-base font-black text-${c.color}-800 mb-5 flex items-center gap-2`}>{c.icon} {c.title}</h4>
                  <ul className="space-y-2">
                    {c.data.length ? c.data.map((s,i) => (
                      <li key={s.id} className={`text-sm font-black text-${c.color}-900 bg-white/70 px-3 py-2 rounded-2xl flex justify-between shadow-sm`}>
                        <span>{i+1}. {s.name}</span><span className={`text-${c.color}-600`}>{s[c.key]}{c.unit}</span>
                      </li>
                    )) : <li className={`text-sm font-bold text-${c.color}-400 text-center py-4`}>데이터 없음</li>}
                  </ul>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-6 mb-6 border-b-4 border-amber-200/50 pb-6 mt-12">
              <div>
                <h2 className="text-3xl font-black text-amber-900 flex items-center gap-3"><Users className="text-amber-500 w-9 h-9"/> 우리 반 꼬마 시민들</h2>
                <p className="text-xs font-bold text-slate-500 mt-2">
                  {todayIdx < 0 ? '🔔 주말입니다.' : attendOpen ? '🕗 출석 체크 가능 (08:30까지)' : '⏰ 출석 시간이 마감되었습니다'}
                </p>
              </div>
              <button onClick={() => setShowPraiseModal(true)} className="w-full sm:w-auto bg-gradient-to-r from-pink-400 to-rose-500 text-white px-8 py-4 rounded-full font-black text-base shadow-[0_10px_20px_rgba(244,63,94,0.3)] hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-3 border-2 border-pink-300">
                <Heart className="w-5 h-5 fill-white animate-pulse"/> 온기 우체통
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {sortedDashboardStats.map(s => {
                ...
                return (
                  <div key={s.id} className={`p-5 rounded-[30px] border-4 shadow-sm transition-all relative flex flex-col bg-white hover:shadow-xl ${s.status === 'crisis' ? 'border-slate-300 bg-slate-100 opacity-60 grayscale' : (s.status === 'pending' ? 'border-orange-300 bg-orange-50' : 'border-white hover:border-amber-300')}`}>

                    {db.timeAttack?.isActive && s.status !== 'crisis' && (
                      <div className="absolute -top-3 -right-3 z-20">
                        <button onClick={() => toggleTimeAttackClear(s.id)} className={`flex items-center gap-1 px-2 py-1 rounded-full font-black text-[10px] shadow-md border-2 ${isTaCleared ? 'bg-green-500 text-white border-green-600 scale-110' : 'bg-white text-slate-400 border-slate-200'}`}>
                          {isTaCleared ? <><CheckCircle2 className="w-3 h-3"/>완료</> : <><Timer className="w-3 h-3"/>도전</>}
                        </button>
                      </div>
                    )}
                    ...
                    {isAnim && (
                      <div className="absolute top-6 left-1/2 -translate-x-1/2 z-30 pointer-events-none">
                        <span className="text-3xl font-black text-amber-500 drop-shadow-lg animate-flyToScore">+1</span>
                      </div>
                    )}

                    <div className="flex justify-between items-center mb-4 border-b-2 border-slate-100/50 pb-3">
                      <div className="flex items-center gap-2.5">
                        <span className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-base shadow-inner ${s.status === 'crisis' ? 'bg-slate-200 text-slate-500' : 'bg-amber-100 text-amber-800'}`}>{s.id}</span>
                        <div>
                          <p className="text-[9px] font-black text-slate-400 uppercase">Coins</p>
                          <p className={`font-black text-xl leading-none ${s.status === 'crisis' ? 'text-slate-500' : 'text-amber-600'}`}>{s.coins}🪙</p>
                        </div>
                      </div>
                      {isAuthenticated === 'teacher' && (
                        <button onClick={() => openNoteModal(s.id)} className="p-2 rounded-xl bg-yellow-50 text-yellow-600 hover:bg-yellow-100 border border-yellow-200" title="누가기록">
                    ...
                    <div className="flex flex-col mb-4 gap-2">
                      <p className="text-[11px] font-bold text-slate-400 tracking-wide truncate">{s.group}모둠 · {s.role}</p>

                      {/* 🆕 이름 버튼 (출석 토글) */}
                      <button
                        onClick={() => toggleAttendance(s.id)}
                        disabled={s.status === 'crisis' || (todayIdx < 0)}
                        className={`text-left transition-all ${s.status === 'crisis' ? 'cursor-not-allowed' : 'hover:bg-amber-50 active:scale-95'} rounded-xl px-2 py-1`}
                        title={s.attendedToday ? "출석 취소" : "출석 체크"}
                      >
                        <h3 className={`text-lg font-black flex items-center gap-1 whitespace-nowrap tracking-tight truncate ${s.exp >= 20 && s.status !== 'crisis' ? 'text-amber-700' : nameColor}`}>
                          {s.attendedToday && <span className="text-green-500">✓</span>}
                          {s.name}
                          {s.isLeader && <Crown className="w-3.5 h-3.5 text-amber-400 fill-amber-400"/>}
                          <span className="text-[10px] font-bold text-slate-400 ml-1">{s.weeklyCount}/5 · ⭐{s.streak}</span>
                        </h3>
                      </button>

                      <div className={`text-xs font-black px-2 py-1 rounded-xl border-2 self-start ${s.status === 'crisis' ? 'bg-slate-200 border-slate-300 text-slate-500' : `${s.mastery.bg} ${s.mastery.color}`}`}>
                        {s.mastery.label} ({s.exp})
                      </div>
                    </div>

                    {db.settings?.showCumulativeStats && (
                      <div className="bg-slate-50 border border-slate-200 p-2.5 rounded-2xl mb-3 text-[10px] font-bold text-slate-600 grid grid-cols-2 gap-1.5">
                        <span>✅{s.atExp}</span><span>💎{s.atDonate}</span>
                        <span>🚀{s.atFund}</span><span>🚨{s.atPen}</span>
                      </div>
                    )}

                    <div className="mt-auto pt-1">
                      {s.status === 'normal' && <button onClick={() => handleGivePenalty(s.id)} className="w-full py-3 bg-slate-50 border border-slate-200 hover:bg-red-50 text-slate-400 hover:text-red-600 rounded-2xl font-black text-xs flex items-center justify-center gap-1.5"><AlertTriangle className="w-3.5 h-3.5"/> 위기 지정</button>}
                      {s.status === 'crisis' && <p className="text-center font-black text-white bg-slate-600 py-3 rounded-2xl text-xs">🚨 성찰 요망</p>}
                      {s.status === 'pending' && <p className="text-center font-black text-orange-800 bg-orange-200 py-3 rounded-2xl text-xs">⏳ 승인 대기</p>}
                    </div>
                  </div>
                );
              })}
              ...
              <div className="text-left space-y-8 bg-emerald-50/60 p-8 md:p-10 rounded-[36px] border-2 border-emerald-100">
                ...
                {refTarget && (
                  <div className="bg-gradient-to-r from-pink-50 to-rose-50 border-2 border-pink-200 p-6 rounded-[26px] text-pink-800">
                    <h4 className="text-base font-black mb-3 flex items-center gap-2"><Heart className="w-5 h-5 fill-pink-400"/> 응원합니다.</h4>
                    {randomPraise ? (
                ...
                <div>
                  <label className="block text-sm font-black mb-3 text-emerald-800 bg-emerald-100 inline-block px-4 py-1.5 rounded-full">2. 어떤 역량이 필요할까요?</label>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {SEL_OPTIONS.map(opt => (
                      <button key={opt.id} onClick={() => setRefTag(opt.name)} className={`p-4 rounded-2xl border-2 font-black text-xs text-left transition-all ${refTag === opt.name ? 'bg-emerald-500 border-emerald-500 text-white scale-105' : 'bg-white border-white text-slate-500'}`}>{opt.name}</button>
                    ))}
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-black mb-3 text-emerald-800 bg-emerald-100 inline-block px-4 py-1.5 rounded-full">3. 마음의 다짐</label>
                  <textarea value={refText} onChange={e=>setRefText(e.target.value)} rows="6" className="w-full p-6 rounded-[26px] border-2 border-white font-black bg-white resize-none text-sm outline-none focus:border-emerald-300" placeholder={refTag ? SEL_GUIDES[refTag] : "역량을 먼저 선택해 주세요."}/>
                </div>
            ...
            <section className="flex-1 p-6 lg:p-10 overflow-y-auto bg-slate-50/30">
              ...
              <div className="mb-10 bg-gradient-to-r from-yellow-100 to-amber-100 p-6 rounded-[30px] shadow-sm flex flex-col md:flex-row gap-6 items-center border-2 border-yellow-200">
                <div className="md:w-1/3 text-center md:text-left">
                  ...
                  <p className="text-xs font-bold text-amber-700">코인으로 명성을 높이세요!</p>
                </div>
                ...
                <div className="space-y-8 animate-in fade-in">
                  ...
                  <div className="bg-white border-2 border-indigo-50 rounded-[30px] overflow-hidden shadow-sm">
                    <table className="w-full text-left">
                      <thead className="bg-indigo-50/50 text-xs font-black text-indigo-400 uppercase">
                      ...
                      <tbody className="divide-y divide-indigo-50/50">
                        {allStats.map(s => (
                          <tr key={s.id} className="hover:bg-indigo-50/20">
                            <td className="p-4 font-black text-base text-slate-700">{s.name}</td>
                            <td className="p-4">
                            ...
                            <td className="p-4 text-center">
                              <button onClick={() => handleStudentFieldChange(s.id, 'isLeader', !s.isLeader)} className={`px-4 py-2 rounded-lg font-black text-xs ${s.isLeader ? 'bg-amber-400 text-white' : 'bg-slate-100 text-slate-400'}`}>
                          ...
                          <div className="grid grid-cols-1 gap-3">
                            {members.map(s => (
                              <div key={s.id} className="bg-slate-50 p-4 rounded-2xl flex items-center justify-between">
                                <div>
                                  <p className="text-xs font-bold text-slate-400 mb-0.5">{s.role}</p>
                      ...
                      <div key={item.id} className="bg-white p-8 rounded-[30px] shadow-sm border-2 border-slate-100 flex flex-col justify-between">
                        <div>
                          <div className="flex justify-between items-start mb-4">
                            <span className="text-xs font-black bg-slate-100 text-slate-500 px-3 py-1.5 rounded-lg">{item.creator}</span>
                      ...
                      {[
                        ...
                        <div key={q.key} className={`bg-${q.color}-50 p-3 rounded-xl border border-${q.color}-100 flex gap-2`}>
                          <input type="text" value={db.coopQuest?.[`${q.key}Name`] || q.defName}
                                 ...
                                 className="flex-1 p-2.5 rounded-lg text-sm font-bold border-none outline-none"/>
                          <input type="number" value={db.coopQuest?.[q.key] ?? q.defVal}
                                 onChange={e => sync({ coopQuest: { ...db.coopQuest, [q.key]: toInt(e.target.value) } })}
                                 onFocus={lockEditing} onBlur={unlockEditing}
                                 className={`w-16 p-2.5 rounded-lg text-sm font-black text-${q.color}-600 border-none outline-none text-center`}/>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="space-y-4 max-h-[500px] overflow-y-auto pr-3 custom-scrollbar">
                      {safeArray(db.pendingShopItems).filter(i => i && i.name).map(item => (
                        <div key={item.id} className="bg-amber-50 p-5 rounded-2xl border-2 border-amber-200">
                          <div className="flex justify-between mb-2">
                            <span className="font-black text-amber-800 bg-amber-100 px-3 py-1 rounded-lg text-xs">장인: {item.creator}</span>
                            <span className="text-sm font-black text-amber-600">{item.price}🪙</span>
                          </div>
                          <p className="text-base text-slate-800 font-black mb-3">"{item.name}"</p>
                      ...
                      {safeArray(db.pendingReflections).filter(r => r && r.sId).map(r => (
                        <div key={r.id} className="bg-red-50 p-5 rounded-2xl border-2 border-red-200">
                          ...
                          <p className="text-sm text-slate-700 font-bold mb-4 whitespace-pre-wrap bg-white p-3 rounded-xl border border-red-100">"{r.text}"</p>
                          <div className="flex gap-2">
                      ...
                      {safeArray(db.pendingPraises).filter(p => p && p.toId).map(p => {
                        ...
                        return (
                          <div key={p.id} className="bg-pink-50 p-5 rounded-2xl border-2 border-pink-200">
                            ...
                            <p className="text-sm text-slate-700 font-bold mb-4 bg-white p-3 rounded-xl border border-pink-100">"{p.text}"</p>
                            <div className="flex gap-2">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {safeArray(db.shopItems).filter(i => i && i.name).map(item => (
                          <div key={item.id} className="bg-amber-50 p-3 rounded-xl border flex justify-between items-center">
                            <div>
                              <span className="text-[9px] text-slate-400 font-black bg-white px-2 py-0.5 rounded">{item.creator}</span>
                              <h4 className="font-black text-slate-800 mt-1">{item.name}</h4>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {safeArray(db.funding).filter(f => f && f.name).map(f => (
                          <div key={f.id} className="bg-white p-4 rounded-xl border border-blue-200 flex justify-between items-center">
                            <div><h4 className="font-black text-blue-900">{f.name}</h4><p className="text-blue-500 font-bold text-xs">{f.current}/{f.target}p</p></div>
                  ...
                  <div className="bg-white p-8 rounded-[30px] shadow-sm border border-slate-100 flex flex-col md:flex-row gap-6">
                    <div className="w-full md:w-1/3 space-y-4">
                      ...
                      <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100 space-y-2">
                        ...
                        <button disabled={!selectedReportStudent}
                        ...
                        <p className="text-[10px] font-bold text-indigo-400 leading-tight">클립보드 복사 + 파일 다운로드됩니다. ChatGPT/Claude에 붙여넣으세요.</p>
                      </div>
                        ...
                        return (
                          <div>
                            ...
                            <div className="border-t-2 border-slate-200 pt-5 mt-5">
                              <button onClick={() => setShowNotesInReport(!showNotesInReport)}
                                      className="w-full bg-yellow-100 hover:bg-yellow-200 text-yellow-800 font-black p-3 rounded-xl flex items-center justify-between transition-colors">
                                <span className="flex items-center gap-2"><StickyNote className="w-5 h-5"/> 누가기록 ({s.notes.length}건)</span>
                                <span>{showNotesInReport ? '▲' : '▼'}</span>
                              </button>
                              {showNotesInReport && (
                                <div className="mt-3 space-y-2 animate-in fade-in">
                                  ...
                                  {s.notes.map(n => (
                                    <div key={n.id} className="bg-white p-3 rounded-xl border border-yellow-200 flex justify-between items-start gap-3">
                                      <div>
                                        <p className="text-[10px] font-black text-yellow-600">{n.date}</p>
                                        <p className="text-sm font-bold text-slate-700 mt-1 whitespace-pre-wrap">{n.text}</p>
                                      </div>
                                  ...
                                  <button onClick={() => openNoteModal(s.id)} className="w-full bg-yellow-500 text-white p-3 rounded-xl font-black text-sm flex items-center justify-center gap-2 hover:bg-yellow-600">
                ...
                <div className="space-y-6 animate-in fade-in">
                  ...
                  <div className="bg-white p-8 rounded-[30px] shadow-sm border">
                    <div className="bg-amber-50 p-6 rounded-2xl border-2 border-amber-200 mb-6">
                      ...
                      <button onClick={teacherAddHoliday} className="w-full bg-amber-500 text-white py-4 rounded-xl font-black text-lg hover:bg-amber-600">
                        전체 출석일수 +1 (공휴일 적용)
                      </button>
                      <p className="text-[11px] font-bold text-amber-500 mt-3 leading-relaxed">
                        💡 이 버튼을 누르면 모든 학생의 이번 주 출석일수에 +1이 더해지고, 그 결과 5일이 된 학생에게 자동으로 🪙3 개근 보너스가 지급됩니다.
                      </p>
                    </div>
                ...
                <div className="space-y-6 animate-in fade-in">
                  ...
                  <div className="bg-white p-8 rounded-[30px] shadow-sm border space-y-8">
                    ...
                    <div className="bg-slate-50 p-6 rounded-[26px] border grid grid-cols-1 md:grid-cols-2 gap-8">
                      {[
                        ...
                        <div key={p.field}>
                          ...
                          <p className="text-xs font-bold text-slate-400 mt-2">현재: {p.current}</p>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="bg-red-50 p-5 rounded-2xl border-2 border-red-200">
                      ...
                      <p className="text-xs font-bold text-red-600 mb-3">유출 우려 시 누르세요. 본인 포함 전원 로그아웃됩니다.</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t-2">
                      ...
                      <div>
                        ...
                        <input type="number" value={Math.floor((db.settings?.defaultBreakMs || DEFAULT_BREAK_MS)/60000)}
                               onChange={e => sync({ settings: { ...db.settings, defaultBreakMs: toInt(e.target.value, 10)*60000 } })}
                               onFocus={lockEditing} onBlur={unlockEditing}
                               className="w-full p-5 rounded-2xl bg-slate-50 border font-black text-lg outline-none"/>
                      </div>
                    </div>
                    ...
                    <div className="pt-6 border-t-2">
                      <button onClick={toggleCumulativeStats} className={`w-full py-4 rounded-2xl font-black text-lg shadow-md flex items-center justify-center gap-3 ${db.settings?.showCumulativeStats ? 'bg-blue-600 text-white' : 'bg-white text-slate-500 border-2'}`}>
                        <Eye className="w-5 h-5"/> 누적 스탯 표시: {db.settings?.showCumulativeStats ? 'ON' : 'OFF'}
                      </button>
                    </div>
                        ...
                        {[
                          ...
                          <div key={f.key} className="bg-white p-4 rounded-xl shadow-sm">
                            ...
                            <input type="number"
                                   ...
                                   onFocus={lockEditing} onBlur={unlockEditing}
                                   className={`w-full p-3 rounded-lg border border-${f.color}-200 font-black text-${f.color}-600 outline-none`}/>
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      {showNoteModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-[1000] p-4">
          <div className="bg-white p-10 rounded-[40px] w-full max-w-lg shadow-2xl animate-in zoom-in-95 border-4 border-yellow-200">
            ...
            <p className="text-sm font-bold text-yellow-600 mb-6">
              {allStats.find(s => s.id === showNoteModal)?.name} · {formatDate()}
            </p>
            <textarea value={noteText} onChange={e=>setNoteText(e.target.value)} rows="6"
                      placeholder="오늘의 관찰·일화·지도사항을 간단히 기록하세요."
                      autoFocus
                      className="w-full p-5 rounded-[26px] bg-yellow-50 border-2 border-yellow-200 font-black text-base outline-none text-yellow-900 shadow-inner mb-6"/>
            <div className="flex gap-3">
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center p-6 z-[9999]">
          <div className="bg-white rounded-[50px] p-12 w-full max-w-lg text-center shadow-2xl animate-in zoom-in-95 border-4 border-blue-100">
            ...
            <input type="password" value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==='Enter' && handleLogin()}
                   className="w-full text-center text-5xl tracking-[15px] font-black p-6 border-4 rounded-[30px] outline-none mb-8 bg-slate-50" autoFocus/>
            <div className="flex gap-3">
      ...
      {/* ═══ NAV BAR ═══ */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t-4 border-amber-100 px-4 py-4 flex justify-around items-center z-[5000] shadow-[0_-15px_50px_rgba(0,0,0,0.08)] pb-6 print:hidden">
        {[
          { id: 'dashboard', icon: <Target className="w-7 h-7"/>, label: db.settings?.menuNames?.[0] || "현황판", color: "text-blue-500" },
          ...
          { id: 'admin', icon: <Settings className="w-7 h-7"/>, label: db.settings?.menuNames?.[3] || "관리실", color: "text-slate-600" }
        ].map(item => (
          <button key={item.id}
                  onClick={() => {
                    if (item.id === 'admin') {
                  ...
                  className={`flex flex-col items-center gap-1.5 flex-1 transition-all duration-300 ${activeTab === item.id ? `${item.color} scale-110 -translate-y-3 drop-shadow-lg` : 'text-slate-400 opacity-60'}`}>
            {item.icon}
...
function TimerWidget({ status, display, timer, warningLevel, breakInput, setBreakInput, defaultBreakMin, onStopwatch, onCountdown, onPause, onResume, onReset, onBreak, lockEditing, unlockEditing }) {
  ...
  return (
    <div className={`p-5 rounded-3xl border-4 shadow-lg backdrop-blur-sm ${bgClass} ${flashClass}`}>
      <div className="flex items-center gap-2 mb-3">
        {isBreak ? <Coffee className="w-5 h-5 text-emerald-600"/> : <Timer className="w-5 h-5 text-slate-600"/>}
        <span className="text-[11px] font-black uppercase tracking-wider text-slate-600">
          {isBreak ? '쉬는시간' : (status === 'class_sw' ? '스톱워치' : status === 'class_cd' ? '카운트다운' : '수업 중')}
        </span>
      </div>

      <div className={`text-center py-3 rounded-2xl mb-3 ${isBreak && warningLevel >= 2 ? 'bg-red-500 text-white' : 'bg-white'}`}>
        <span className="text-5xl md:text-6xl font-black tracking-widest tabular-nums">{display}</span>
      </div>
      ...
      {status === 'idle' && (
        <div className="space-y-2">
          <div className="grid grid-cols-4 gap-1.5">
            {[1, 3, 5, 10].map(m => (
              <button key={m} onClick={() => onCountdown(m)}
                      className="py-2 bg-slate-100 hover:bg-slate-200 rounded-lg font-black text-xs">{m}분</button>
            ))}
          </div>
          <button onClick={onStopwatch} className="w-full py-2 bg-indigo-500 text-white rounded-lg font-black text-xs flex items-center justify-center gap-1">
            <Play className="w-3.5 h-3.5"/> 스톱워치 시작
          </button>
          <div className="flex gap-1.5 pt-2 border-t border-white/60">
            <input type="number" value={breakInput} onChange={e=>setBreakInput(e.target.value)}
                   onFocus={lockEditing} onBlur={unlockEditing}
                   className="w-14 p-2 rounded-lg border text-center font-black text-sm outline-none"/>
            <button onClick={() => onBreak(Math.max(1, toInt(breakInput, defaultBreakMin)))}
                    className="flex-1 py-2 bg-emerald-500 text-white rounded-lg font-black text-xs flex items-center justify-center gap-1">
              <Coffee className="w-3.5 h-3.5"/> 쉬는시간 시작
            </button>
          </div>
        </div>
      )}
      ...
      {status !== 'idle' && (
        <div className="flex gap-2">
          {isRunning
            ? <button onClick={onPause} className="flex-1 py-2.5 bg-yellow-500 text-white rounded-lg font-black text-sm flex items-center justify-center gap-1"><Pause className="w-4 h-4"/> 일시정지</button>
            : <button onClick={onResume} className="flex-1 py-2.5 bg-green-500 text-white rounded-lg font-black text-sm flex items-center justify-center gap-1"><Play className="w-4 h-4"/> 재개</button>}
          <button onClick={onReset} className="flex-1 py-2.5 bg-slate-400 text-white rounded-lg font-black text-sm flex items-center justify-center gap-1"><RotateCcw className="w-4 h-4"/> 리셋</button>
        </div>
      )}
    </div>
  );
