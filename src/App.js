...
  // ✅ FIX D: db를 ref로도 유지하여 stale closure 방지
  const dbRef = useRef(INITIAL_DB);

  // ✅ FIX D: authRevokedAt이 실제 로드된 뒤에도 재검증
  useEffect(() => {
    if (isLoading) return; // Firebase 데이터 로드 완료 후에 검증
    const saved = loadAuth(db.settings?.authRevokedAt || 0);
    if (saved) {
      setIsAuthenticated(saved);
    } else {
      // 세션 만료 또는 무효화된 경우 강제 로그아웃
...
            };
            dbRef.current = next; // ✅ FIX D: ref 동기화
            return next;
          });
        }
      } catch (_) {}
...
  // ✅ FIX A: 출석 주간 정리 - isLoading이 false로 바뀐 직후 1회만 실행
  // deps에 isLoading만 넣되, 실제 db 데이터는 dbRef를 통해 최신값 참조
  useEffect(() => {
    if (isLoading) return;
...
  // ✅ FIX B: 타이머 useEffect - stale closure 방지를 위해 dbRef 사용
  useEffect(() => {
    const tick = () => {
      const t = db.timer || {};
...
          // ✅ FIX B: dbRef를 통해 최신 db 참조 (stale closure 방지)
...
      // ✅ FIX E: currentDb.manualRepOffset으로 최신값 참조
...
    const currentDb = dbRef.current; // ✅ FIX E
...
    setShowNoteModal(null); setNoteText(""); alert("📝 누가기록 저장 완료!");
  };
...
    const ms = minutes * 60 * 1000; lastNotifiedRef.current = {};
...
    sync({ roleExp: { ...db.roleExp, [id]: Math.max(0, (db.roleExp[id]||0) + delta) }, allTime: { ...db.allTime, exp: { ...db.allTime.exp, [id]: Math.max(0, (db.allTime.exp?.[id]||0) + delta) } } });
  };
  const handleGivePenalty = (id) => {
    if (!isAuthenticated) return setShowModal('password');
    if (!window.confirm("위기 지정할까요?")) return;
    playSound('bad');
    sync({ studentStatus: { ...db.studentStatus, [id]: 'crisis' }, penaltyCount: { ...db.penaltyCount, [id]: (db.penaltyCount[id] || 0) + 1 }, allTime: { ...db.allTime, penalty: { ...db.allTime.penalty, [id]: (db.allTime.penalty?.[id] || 0) + 1 } } });
  };
  const handleDonate = (sId, amount) => {
    const u = allStats.find(s => s.id == sId); if (!u || u.coins < amount) return alert("코인 부족!");
    playSound('buy');
    sync({ usedCoins: { ...db.usedCoins, [sId]: (db.usedCoins[sId] || 0) + amount }, donations: [{ id: Date.now(), name: u.name, amount }, ...safeArray(db.donations)].slice(0, 15), allTime: { ...db.allTime, donate: { ...db.allTime.donate, [sId]: (db.allTime.donate?.[sId] || 0) + amount } } });
    alert("기부 완료! ✨");
  };
  const handleFund = (fId, sId, amount) => {
    const u = allStats.find(s => s.id == sId); if (!u || u.coins < amount) return alert("코인 부족!");
    playSound('buy');
    sync({ usedCoins: { ...db.usedCoins, [sId]: (db.usedCoins[sId] || 0) + amount }, funding: safeArray(db.funding).map(f => f.id === fId ? { ...f, current: (Number(f.current)||0) + amount } : f), allTime: { ...db.allTime, fund: { ...db.allTime.fund, [sId]: (db.allTime.fund?.[sId] || 0) + amount } } });
    alert("투자 완료!");
  };
...
    setArtisanTarget(""); setArtisanItemName(""); setArtisanItemPrice(""); alert("결재 올림!");
  };
  const submitPraise = () => {
...
    setShowPraiseModal(false); setPraiseTarget(""); setPraiseText(""); setPraiseTag(""); alert("온기 배달 완료!");
  };
  const submitReflection = () => {
    if (!refTarget || !refTag || !refText) return alert("빈칸 확인!");
    sync({ pendingReflections: [{ id: Date.now(), sId: refTarget, tag: refTag, text: refText, date: formatDate() }, ...safeArray(db.pendingReflections)], studentStatus: { ...db.studentStatus, [refTarget]: 'pending' } });
    setRefTarget(""); setRefText(""); setRefTag(""); alert("다짐 제출 완료!");
  };
...
    }
    sync(updates); alert(`승인 완료! (+${earned}🪙)`); playSound('good');
  };
...
    if (!window.confirm("모든 기기에서 강제 로그아웃합니다.")) return;
...
    sync({ roleExp: {}, bonusCoins: {}, usedCoins: {}, penaltyCount: {}, studentStatus: {}, pendingReflections: [], pendingPraises: [], approvedPraises: [], donations: [], pendingShopItems: [], shopItems: [], funding: [], manualRepOffset: 0, allTime: { exp: {}, penalty: {}, donate: {}, fund: {} }, timeAttack: { isActive: false, title: "", rewardRep: 100, endTime: null, cleared: [] }, attendance: {}, attendanceBonus: {}, streakWeeks: {}, notes: {}, extraAttendDays: 0 });
    alert("전체 리셋 완료.");
  };
...
                <span className="text-7xl md:text-8xl font-black text-amber-900 drop-shadow-sm tracking-tighter">{classReputation}</span>
...
              <div className="scale-125 transform-origin-center">{renderEvolution(evolutionLevel)}</div>
            </div>
...
                  <button onClick={() => addCoopScore(db.coopQuest?.q3 || 20, db.coopQuest?.q3Name || "전담 우수")} className="bg-emerald-50 hover:bg-emerald-100 text-emerald-700 py-4 rounded-2xl font-black text-sm border border-emerald-200 active:scale-95 col-span-2 truncate px-2">{db.coopQuest?.q3Name || "전담 우수"} +{db.coopQuest?.q3 || 20}</button>
                </div>
...
            {/* ✅ FIX C: 동적 Tailwind 클래스 → 하드코딩으로 교체 (프로덕션 purge 대응) */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {/* 역할 완수 TOP5 - blue */}
              <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-8 rounded-[32px] shadow-sm border border-blue-200">
                <h4 className="text-lg font-black text-blue-800 mb-6 flex items-center gap-2"><CheckCircle2 className="w-6 h-6"/> 🏆 역할 완수 TOP5</h4>
                <ul className="space-y-3">
                  {topExp.length ? topExp.map((s,i) => (
                    <li key={s.id} className="text-base font-black text-blue-900 bg-white/70 px-4 py-2.5 rounded-2xl flex justify-between shadow-sm">
                      <span>{i+1}. {s.name}</span><span className="text-blue-600">{s.atExp}회</span>
                    </li>
                  )) : <li className="text-sm font-bold text-blue-400 text-center py-5">데이터 없음</li>}
                </ul>
              </div>
              {/* 기부 천사 TOP5 - amber */}
              <div className="bg-gradient-to-br from-amber-50 to-amber-100 p-8 rounded-[32px] shadow-sm border border-amber-200">
                <h4 className="text-lg font-black text-amber-800 mb-6 flex items-center gap-2"><Coins className="w-6 h-6"/> 🏆 기부 천사 TOP5</h4>
                <ul className="space-y-3">
                  {topDonate.length ? topDonate.map((s,i) => (
                    <li key={s.id} className="text-base font-black text-amber-900 bg-white/70 px-4 py-2.5 rounded-2xl flex justify-between shadow-sm">
                      <span>{i+1}. {s.name}</span><span className="text-amber-600">{s.atDonate}🪙</span>
                    </li>
                  )) : <li className="text-sm font-bold text-amber-400 text-center py-5">데이터 없음</li>}
                </ul>
              </div>
              {/* 펀딩 기여 TOP5 - pink */}
              <div className="bg-gradient-to-br from-pink-50 to-pink-100 p-8 rounded-[32px] shadow-sm border border-pink-200">
                <h4 className="text-lg font-black text-pink-800 mb-6 flex items-center gap-2"><Target className="w-6 h-6"/> 🏆 펀딩 기여 TOP5</h4>
                <ul className="space-y-3">
                  {topFund.length ? topFund.map((s,i) => (
                    <li key={s.id} className="text-base font-black text-pink-900 bg-white/70 px-4 py-2.5 rounded-2xl flex justify-between shadow-sm">
                      <span>{i+1}. {s.name}</span><span className="text-pink-600">{s.atFund}🪙</span>
                    </li>
                  )) : <li className="text-sm font-bold text-pink-400 text-center py-5">데이터 없음</li>}
                </ul>
              </div>
            </div>
...
              </div>
              <button onClick={() => setHelpSubTab('inspector')} className={`w-full p-5 rounded-2xl font-black text-left flex items-center gap-3 text-base ${helpSubTab === 'inspector' ? 'bg-indigo-500 text-white shadow-xl' : 'bg-white text-indigo-400'}`}><Briefcase className="w-5 h-5"/> 감찰사 본부</button>
              <button onClick={() => setHelpSubTab('magistrate')} className={`w-full p-5 rounded-2xl font-black text-left flex items-center gap-3 text-base ${helpSubTab === 'magistrate' ? 'bg-indigo-500 text-white shadow-xl' : 'bg-white text-indigo-400'}`}><BookOpen className="w-5 h-5"/> 현령 관리소</button>
              <button onClick={() => setHelpSubTab('shop')} className={`w-full p-5 rounded-2xl font-black text-left flex items-center gap-3 text-base ${helpSubTab === 'shop' ? 'bg-amber-400 text-white shadow-xl' : 'bg-white text-amber-500'}`}><ShoppingCart className="w-5 h-5"/> 학급 상점</button>
            </aside>
...
                    <div className={`px-8 py-3 rounded-full font-black text-base shadow-lg border-4 ${isShopOpen ? 'bg-green-500 text-white border-green-300 animate-pulse' : 'bg-slate-500 text-white border-slate-400'}`}>{isShopOpen ? "🔓 영업 중" : "🔒 목요일 개방"}</div>
                  </div>
...
                            <div className="flex gap-2">
                              <button onClick={() => approvePraise(p)} className={`flex-1 py-3 rounded-lg font-black text-sm ${isCrisis ? 'bg-slate-300 text-slate-500 cursor-not-allowed' : 'bg-pink-500 text-white hover:bg-pink-600'}`}>승인</button>
...
                      <div>
                        <h4 className="font-black text-slate-700 mb-3 flex items-center gap-2"><Lock className="w-5 h-5 text-blue-500"/> 관리자 비번</h4>
                        <div className="flex gap-2">
                          <input type="password" value={masterPwInput} onChange={e=>setMasterPwInput(e.target.value)} onFocus={lockEditing} onBlur={unlockEditing} placeholder="새 비번" className="flex-1 p-3 rounded-xl border font-black outline-none"/>
                          <button onClick={() => { if (!masterPwInput) return alert('입력!'); sync({ settings: { ...db.settings, masterPw: masterPwInput } }); alert('변경!'); setMasterPwInput(''); }} className="bg-blue-600 text-white px-5 rounded-xl font-black">변경</button>
                        </div>
                        <p className="text-xs font-bold text-slate-400 mt-2">현재: {String(db.settings?.masterPw)}</p>
                      </div>
                      <div>
                        <h4 className="font-black text-slate-700 mb-3 flex items-center gap-2"><Lock className="w-5 h-5 text-indigo-500"/> 도움실 비번</h4>
                        <div className="flex gap-2">
                          <input type="password" value={helpPwInput} onChange={e=>setHelpPwInput(e.target.value)} onFocus={lockEditing} onBlur={unlockEditing} placeholder="새 비번" className="flex-1 p-3 rounded-xl border font-black outline-none"/>
                          <button onClick={() => { if (!helpPwInput) return alert('입력!'); sync({ settings: { ...db.settings, helpRoomPw: helpPwInput } }); alert('변경!'); setHelpPwInput(''); }} className="bg-indigo-500 text-white px-5 rounded-xl font-black">변경</button>
                        </div>
                        <p className="text-xs font-bold text-slate-400 mt-2">현재: {String(db.settings?.helpRoomPw)}</p>
                      </div>
                    </div>
...
                          <label className="block text-xs font-black text-indigo-600 mb-2">최고 목표</label>
                          <input type="number" value={db.settings?.targetScore ?? 5000} onChange={e => sync({ settings: { ...db.settings, targetScore: toInt(e.target.value, 5000) } })} onFocus={lockEditing} onBlur={unlockEditing} className="w-full p-3 rounded-lg border border-indigo-200 font-black text-indigo-600 outline-none"/>
                        </div>
                        <div className="bg-white p-4 rounded-xl shadow-sm">
                          <label className="block text-xs font-black text-indigo-600 mb-2">기본 코인</label>
                          <input type="number" value={db.settings?.pointConfig?.praiseBasic ?? 10} onChange={e => sync({ settings: { ...db.settings, pointConfig: { ...db.settings?.pointConfig, praiseBasic: toInt(e.target.value, 10) } } })} onFocus={lockEditing} onBlur={unlockEditing} className="w-full p-3 rounded-lg border border-indigo-200 font-black text-indigo-600 outline-none"/>
                        </div>
                        <div className="bg-white p-4 rounded-xl shadow-sm">
                          <label className="block text-xs font-black text-pink-500 mb-2">테마 보너스</label>
                          <input type="number" value={db.settings?.pointConfig?.praiseBonus ?? 15} onChange={e => sync({ settings: { ...db.settings, pointConfig: { ...db.settings?.pointConfig, praiseBonus: toInt(e.target.value, 15) } } })} onFocus={lockEditing} onBlur={unlockEditing} className="w-full p-3 rounded-lg border border-pink-200 font-black text-pink-600 outline-none"/>
                        </div>
                        <div className="bg-white p-4 rounded-xl shadow-sm">
                          <label className="block text-xs font-black text-red-500 mb-2">위기 차감</label>
                          <input type="number" value={db.settings?.pointConfig?.penalty ?? 20} onChange={e => sync({ settings: { ...db.settings, pointConfig: { ...db.settings?.pointConfig, penalty: toInt(e.target.value, 20) } } })} onFocus={lockEditing} onBlur={unlockEditing} className="w-full p-3 rounded-lg border border-red-200 font-black text-red-600 outline-none"/>
                        </div>
                      </div>
                    </div>
                  </div>
...
          <button onClick={onReset} className="flex-1 py-4 bg-slate-400 text-white rounded-xl font-black text-base flex items-center justify-center gap-2 hover:bg-slate-500 transition-colors"><RotateCcw className="w-5 h-5"/> 리셋</button>
        </div>
      )}
    </div>
  );
