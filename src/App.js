...
const getWeekKey = (d = new Date()) => { const date = new Date(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate())); const day = date.getUTCDay() || 7; date.setUTCDate(date.getUTCDate() + 4 - day); const yearStart = new Date(Date.UTC(date.getUTCFullYear(), 0, 1)); const weekNo = Math.ceil((((date - yearStart) / 86400000) + 1) / 7); return `${date.getUTCFullYear()}-W${pad2(weekNo)}`; };
const getTodayWeekdayIdx = () => { const d = new Date().getDay(); if (d === 0 || d === 6) return -1; return d - 1; };
const isAttendanceOpen = () => { const now = new Date(); const cutoff = now.getHours() * 60 + now.getMinutes(); return cutoff < (ATTENDANCE_DEADLINE.hour * 60 + ATTENDANCE_DEADLINE.minute); };
const formatDate = (ts = Date.now()) => { const d = new Date(ts); return `${d.getFullYear()}-${pad2(d.getMonth()+1)}-${pad2(d.getDate())}`; };
const formatMs = (ms) => { if (ms < 0) ms = 0; const s = Math.floor(ms / 1000); return `${pad2(Math.floor(s/60))}:${pad2(s%60)}`; };
...
const playSound = (type) => {
  try {
    ...
    const tone = (freq, dur, t0 = 0, waveType = 'sine', volume = 0.15) => { const osc = ctx.createOscillator(); const gain = ctx.createGain(); osc.connect(gain); gain.connect(ctx.destination); osc.type = waveType; osc.frequency.setValueAtTime(freq, now + t0); gain.gain.setValueAtTime(volume, now + t0); gain.gain.exponentialRampToValueAtTime(0.001, now + t0 + dur); osc.start(now + t0); osc.stop(now + t0 + dur); };
    switch (type) {
...
// 🔐 AUTH (localStorage)
// ══════════════════════════════════════════════════════════════
const saveAuth = (role) => { const data = { role, issuedAt: Date.now(), expires: role === 'teacher' ? null : Date.now() + 24*60*60*1000 }; try { localStorage.setItem(AUTH_KEY, JSON.stringify(data)); } catch(_) {} };
const loadAuth = (revokedAt) => { try { const raw = localStorage.getItem(AUTH_KEY); if (!raw) return false; const data = JSON.parse(raw); if (!data || (data.expires !== null && Date.now() > data.expires) || (revokedAt && data.issuedAt < revokedAt)) { localStorage.removeItem(AUTH_KEY); return false; } return data.role; } catch(_) { return false; } };
...
const ENNEAGRAM_DATA = {
  "1": { name: '1번(개혁가)', desc: '규칙과 책임을 잘 지켜요. 결과보다 과정의 꼼꼼함과 정직함을 알아주세요.' }, "2": { name: '2번(조력자)', desc: '관계와 배려를 중시해요. "네 덕분에 고마워"라는 진심 어린 인사가 가장 큰 힘이 됩니다.' },
  "3": { name: '3번(성취자)', desc: '목표 지향적이에요. 구체적인 성과와 학급 기여를 명확히 인정해 주세요.' }, "4": { name: '4번(예술가)', desc: '자신만의 개성과 감정을 중시해요. 독창적 아이디어를 존중해 주세요.' },
  "5": { name: '5번(사색가)', desc: '논리와 분석을 좋아해요. 혼자만의 시간과 지적 호기심을 칭찬해 주세요.' }, "6": { name: '6번(충실가)', desc: '안전과 소속감을 중시해요. "우리가 함께한다"는 든든한 지지가 필요합니다.' },
  "7": { name: '7번(열정가)', desc: '재미와 자유를 추구해요. 긍정적 에너지와 호기심을 격려해 주세요.' }, "8": { name: '8번(도전자)', desc: '강한 의지와 리더십. 스스로 결정할 기회와 신뢰를 부여해 주세요.' },
  "9": { name: '9번(평화주의자)', desc: '조화를 원해요. 다그치기보다 편안한 분위기에서 의견을 물어봐 주세요.' }
};

const DEFAULT_STUDENTS = [
  { id: 1, name: '금채율', role: '학급문고 정리', group: 1, isLeader: true, enneagram: '2' }, { id: 2, name: '김라희', role: '우유 배달', group: 1, isLeader: false, enneagram: '9' }, { id: 3, name: '김민지', role: '다툼 중재자', group: 1, isLeader: false, enneagram: '6' }, { id: 4, name: '김수은', role: '생활태도 체크', group: 1, isLeader: false, enneagram: '1' },
  { id: 5, name: '김시우', role: '칠판 정리', group: 2, isLeader: true, enneagram: '3' }, { id: 6, name: '박서정', role: '질서 관리', group: 2, isLeader: false, enneagram: '8' }, { id: 7, name: '이하윤', role: '학급문고 정리', group: 2, isLeader: false, enneagram: '4' }, { id: 8, name: '장세아', role: '문 닫기', group: 2, isLeader: false, enneagram: '7' },
  { id: 9, name: '최예나', role: '우유 배달', group: 3, isLeader: true, enneagram: '' }, { id: 10, name: '허수정', role: '감찰사', group: 3, isLeader: false, enneagram: '' }, { id: 11, name: '황지인', role: '칠판 정리', group: 3, isLeader: false, enneagram: '' }, { id: 12, name: '김도운', role: '생활 배출물 관리', group: 3, isLeader: false, enneagram: '' },
  { id: 13, name: '김윤재', role: '과제 확인', group: 4, isLeader: true, enneagram: '' }, { id: 14, name: '김정현', role: '질서 관리', group: 4, isLeader: false, enneagram: '' }, { id: 15, name: '김태영', role: '복사물 관리', group: 4, isLeader: false, enneagram: '' }, { id: 16, name: '김해준', role: '칠판 정리', group: 4, isLeader: false, enneagram: '' },
  { id: 17, name: '박동민', role: '과제 확인', group: 5, isLeader: true, enneagram: '' }, { id: 18, name: '서이환', role: '가습기 관리', group: 5, isLeader: false, enneagram: '' }, { id: 19, name: '윤호영', role: '우유 배달', group: 5, isLeader: false, enneagram: '' }, { id: 20, name: '이서준', role: '과제 확인', group: 5, isLeader: false, enneagram: '' },
  { id: 21, name: '이승현', role: '신발장 관리', group: 6, isLeader: true, enneagram: '' }, { id: 22, name: '임유성', role: '질서 관리', group: 6, isLeader: false, enneagram: '' }, { id: 23, name: '장세형', role: '다툼 중재자', group: 6, isLeader: false, enneagram: '' }, { id: 24, name: '조승원', role: '부착물 관리', group: 6, isLeader: false, enneagram: '' },
  { id: 25, name: '차민서', role: '신발장 관리', group: 6, isLeader: false, enneagram: '' }, { id: 26, name: '배지훈', role: '문 닫기', group: 6, isLeader: false, enneagram: '' }
];

const SEL_OPTIONS = [ { id: 'sel1', short: '자기 인식', name: '1단계: 자기 인식 (Self-awareness)' }, { id: 'sel2', short: '자기 관리', name: '2단계: 자기 관리 (Self-management)' }, { id: 'sel3', short: '사회적 인식', name: '3단계: 사회적 인식 (Social awareness)' }, { id: 'sel4', short: '관계 기술', name: '4단계: 관계 기술 (Relationship skills)' }, { id: 'sel5', short: '책임있는 결정', name: '5단계: 책임 있는 의사결정 (Responsible decision-making)' } ];
const SEL_GUIDES = { "1단계: 자기 인식 (Self-awareness)": "상황: 그때 내 몸과 마음에서 어떤 느낌이 들었나요?\n다짐: 내 진짜 감정의 원인은 무엇이었고, 나의 어떤 강점을 활용해 이 마음을 보듬어줄 수 있을까요?", "2단계: 자기 관리 (Self-management)": "상황: 화가 나거나 포기하고 싶었을 때 내 행동은 어땠나요?\n다짐: 감정의 파도를 다스리고, 다음에는 어떻게 다르게 행동할지 구체적으로 적어보세요.", "3단계: 사회적 인식 (Social awareness)": "상황: 친구의 표정이나 말투를 보았을 때 친구의 마음은 어땠을 것 같나요?\n공감: 내가 그 친구의 입장이었다면 어떤 따뜻한 말이나 도움이 필요했을지 상상해 보세요.", "4단계: 관계 기술 (Relationship skills)": "상황: 대화나 활동 중 서로 오해가 생기거나 배려가 부족했던 순간을 적어보세요.\n행동: 마법 같은 우정을 다시 이어가기 위해 내가 먼저 할 수 있는 행동은 무엇인가요?", "5단계: 책임 있는 의사결정 (Responsible decision-making)": "상황: 우리 반의 규칙이나 분위기를 흐릴 수 있었던 나의 선택은 무엇이었나요?\n다짐: 나뿐만 아니라 우리 모두를 위해 더 나은 세상을 만드는 바른 선택을 실천해보세요." };
const PRAISE_GUIDES = { "1단계: 자기 인식 (Self-awareness)": "칭찬 예시: 스스로의 장점을 알고 자신감 있게 도전한 모습을 칭찬해요!", "2단계: 자기 관리 (Self-management)": "칭찬 예시: 감정을 잘 조절하고 끝까지 해낸 모습을 칭찬해요!", "3단계: 사회적 인식 (Social awareness)": "칭찬 예시: 친구의 마음을 먼저 알아채고 공감해 준 모습을 칭찬해요!", "4단계: 관계 기술 (Relationship skills)": "칭찬 예시: 친구의 말을 잘 경청하고 배려한 모습을 칭찬해요!", "5단계: 책임 있는 의사결정 (Responsible decision-making)": "칭찬 예시: 학급을 위해 솔선수범하여 바른 선택을 한 모습을 칭찬해요!" };

// 🔥 V10 진화 칭호 배열
const EVOLUTION_TITLES = [ "🌱 희망의 씨앗이 움터요", "🌿 어린 나무가 자라나요", "🌸 우리만의 꽃이 피었어요", "🔥 열정의 기운이 가득해요", "✨ 전설의 세계수로 자라나요", "🌟 전설의 세계수 완성!" ];

const INITIAL_DB = {
  students: DEFAULT_STUDENTS, rolesList: ['학급문고 정리', '우유 배달', '다툼 중재자', '현령', '감찰사'],
  settings: { title: "달보드레 행복 교실 🌸", menuNames: ["행복 현황판", "성찰과 회복", "도움실", "관리실"], targetScore: 5000, forceShopOpen: false, weeklyTheme: "4단계: 관계 기술 (Relationship skills)", masterPw: "6505", helpRoomPw: "1111", showCumulativeStats: false, defaultBreakMs: DEFAULT_BREAK_MS, pointConfig: { praiseBasic: 10, praiseBonus: 15, penalty: 20 }, authRevokedAt: 0 },
  ...
  allTime: { exp: {}, penalty: {}, donate: {}, fund: {} }, attendance: {}, attendanceBonus: {}, streakWeeks: {}, notes: {}, extraAttendDays: 0
};
...
const renderEvolution = (level) => {
  switch (level) {
    case 0: return <div className="flex items-center gap-2 text-emerald-400 animate-pulse"><Leaf className="w-10 h-10"/> <Sparkles className="w-6 h-6 text-yellow-400"/></div>;
    case 1: return <div className="flex items-center gap-2 text-emerald-500 animate-bounce"><TreeDeciduous className="w-14 h-14"/> <Bird className="w-8 h-8 text-orange-400"/></div>;
    case 2: return <div className="flex items-center gap-2 text-pink-400"><TreeDeciduous className="w-16 h-16 fill-pink-200"/> <Bird className="w-10 h-10 text-orange-500 animate-pulse"/></div>;
    case 3: return <div className="flex items-center gap-3 text-yellow-500 drop-shadow-md"><TreeDeciduous className="w-20 h-20 fill-yellow-200"/> <Flame className="w-14 h-14 text-red-500 animate-bounce"/></div>;
    case 4:
    case 5: return <div className="flex items-center gap-3 text-yellow-300 drop-shadow-[0_0_20px_rgba(250,204,21,0.8)]"><TreeDeciduous className="w-24 h-24 fill-yellow-100 animate-pulse"/> <Bird className="w-16 h-16 fill-red-500 text-red-600 animate-bounce"/> <Star className="w-10 h-10 fill-yellow-400 text-yellow-500 animate-spin-slow"/></div>;
    default: return null;
  }
};
...
export default function App() {
  const [isLoading, setIsLoading] = useState(true); const [activeTab, setActiveTab] = useState('dashboard'); const [helpSubTab, setHelpSubTab] = useState('inspector'); const [adminSubTab, setAdminSubTab] = useState('mission'); const [isAuthenticated, setIsAuthenticated] = useState(false); const [password, setPassword] = useState(""); const [showModal, setShowModal] = useState(null); const [showPraiseModal, setShowPraiseModal] = useState(false); const [showRollingPaper, setShowRollingPaper] = useState(null); const [showNoteModal, setShowNoteModal] = useState(null); const [noteText, setNoteText] = useState(""); const [showNotesInReport, setShowNotesInReport] = useState(false); const [timeLeftString, setTimeLeftString] = useState(""); const [attendAnim, setAttendAnim] = useState(null);
  const [praiseTarget, setPraiseTarget] = useState(""); const [praiseTag, setPraiseTag] = useState(""); const [praiseText, setPraiseText] = useState(""); const [refTarget, setRefTarget] = useState(""); const [refTag, setRefTag] = useState(""); const [refText, setRefText] = useState(""); const [newItemName, setNewItemName] = useState(""); const [newItemPrice, setNewItemPrice] = useState(""); const [artisanTarget, setArtisanTarget] = useState(""); const [artisanItemName, setArtisanItemName] = useState(""); const [artisanItemPrice, setArtisanItemPrice] = useState(""); const [selectedReportStudent, setSelectedReportStudent] = useState(""); const [newStudentName, setNewStudentName] = useState(""); const [newStudentGroup, setNewStudentGroup] = useState("1"); const [newStudentEnneagram, setNewStudentEnneagram] = useState(""); const [newFundName, setNewFundName] = useState(""); const [newFundTarget, setNewFundTarget] = useState(""); const [manualScoreInput, setManualScoreInput] = useState(""); const [masterPwInput, setMasterPwInput] = useState(""); const [helpPwInput, setHelpPwInput] = useState(""); const [taTitle, setTaTitle] = useState("바닥 쓰레기 0개 만들기!"); const [taMins, setTaMins] = useState("10"); const [taReward, setTaReward] = useState("100");
  const [timerDisplay, setTimerDisplay] = useState("00:00"); const [timerStatus, setTimerStatus] = useState('idle'); const [breakInput, setBreakInput] = useState("10"); const [breakWarningLevel, setBreakWarningLevel] = useState(0);

  const [db, setDb] = useState(INITIAL_DB);
  const isEditingRef = useRef(false); const lockEditing = () => { isEditingRef.current = true; }; const unlockEditing = () => { isEditingRef.current = false; };
  const lastNotifiedRef = useRef({}); const dbRef = useRef(INITIAL_DB);
  ...
  useEffect(() => {
    ...
    const fetchLive = async () => {
      ...
      try {
        const res = await fetch(`${DATABASE_URL}${DB_PATH}.json`); const data = await res.json();
        if (alive && data) { setDb(prev => { const next = { ...prev, ...data, settings: { ...prev.settings, ...(data.settings || {}) }, allTime: { ...prev.allTime, ...(data.allTime || {}) }, coopQuest: { ...prev.coopQuest, ...(data.coopQuest || {}) }, timeAttack: { ...prev.timeAttack, ...(data.timeAttack || {}) }, timer: { ...prev.timer, ...(data.timer || {}) } }; dbRef.current = next; return next; }); }
      } catch (_) {}
      if (alive) setIsLoading(false);
    };
    fetchLive(); const interval = setInterval(fetchLive, POLL_INTERVAL_MS); return () => { alive = false; clearInterval(interval); };
  }, []);

  useEffect(() => {
    if (isLoading) return;
    const currentDb = dbRef.current; const currentWeek = getWeekKey(); const attendance = currentDb.attendance || {}; const oldKeys = Object.keys(attendance).filter(k => k !== currentWeek);
    if (oldKeys.length === 0) return;
    const newStreaks = { ...(currentDb.streakWeeks || {}) }; const students = safeArray(currentDb.students);
    oldKeys.forEach(k => { const weekData = attendance[k] || {}; students.forEach(s => { const totalDays = (safeArray(weekData[s.id])).length + (currentDb.extraAttendDays || 0); if (totalDays < 5) newStreaks[s.id] = 0; }); });
    const cleanedAttendance = { [currentWeek]: attendance[currentWeek] || {} }; const cleanedBonus = { [currentWeek]: (currentDb.attendanceBonus || {})[currentWeek] || {} };
  ...
  useEffect(() => {
    ...
    const tick = () => { const diff = Math.floor((db.timeAttack.endTime - Date.now()) / 1000); if (diff <= 0) setTimeLeftString("00:00 (종료)"); else setTimeLeftString(`${pad2(Math.floor(diff/60))}:${pad2(diff%60)}`); };
    tick(); const t = setInterval(tick, 1000); return () => clearInterval(t);
  }, [db.timeAttack?.isActive, db.timeAttack?.endTime]);

  useEffect(() => {
    const tick = () => {
      const t = db.timer || {}; const mode = t.mode || 'idle'; setTimerStatus(mode);
      if (mode === 'idle') { setTimerDisplay("00:00"); setBreakWarningLevel(0); return; }
      let ms = 0;
      if (mode === 'class_sw') { ms = t.isRunning ? Date.now() - t.startedAt : (t.pausedElapsed || 0); } else if (mode === 'class_cd' || mode === 'break') { ms = t.isRunning ? (t.endTime - Date.now()) : (t.pausedRemaining || 0); if (ms < 0) ms = 0; }
      setTimerDisplay(formatMs(ms));
      if (mode === 'break' && t.isRunning) {
        const sec = Math.floor(ms / 1000); const key = `${t.endTime}_${sec}`;
        const notify = (level, sound) => { if (lastNotifiedRef.current[key + '_' + level]) return; lastNotifiedRef.current[key + '_' + level] = true; playSound(sound); setBreakWarningLevel(level); };
        if (sec === 180) notify(1, 'softChime'); else if (sec === 60) notify(2, 'softChime'); else if (sec <= 30 && sec > 0 && sec % 5 === 0) notify(3, 'beep'); else if (sec > 180) setBreakWarningLevel(0);
      ...
      } else if (mode === 'class_cd' && t.isRunning && ms <= 0) { playSound('chime'); sync({ timer: { mode: 'idle', startedAt: null, endTime: null, duration: null, isRunning: false, pausedElapsed: null, pausedRemaining: null } }); }
    };
    tick(); const iv = setInterval(tick, 250); return () => clearInterval(iv);
  }, [db.timer]);

  const sync = async (updates) => { setDb(prev => { const next = { ...prev, ...updates }; dbRef.current = next; return next; }); try { await fetch(`${DATABASE_URL}${DB_PATH}.json`, { method: 'PATCH', body: JSON.stringify(updates) }); } catch (_) {} };

  const safeStudents = safeArray(db.students);
  const allStats = useMemo(() => safeStudents.map(s => {
    const exp = db.roleExp[s.id] || 0; const bonus = db.bonusCoins?.[s.id] || 0; const coins = Math.max(0, (exp * 10) + bonus - (db.usedCoins[s.id] || 0));
    let mastery = { label: '🌱 인턴', color: 'text-emerald-700', bg: 'bg-emerald-100 border-emerald-200' }; if (exp >= 20) mastery = { label: '👑 장인', color: 'text-amber-700', bg: 'bg-gradient-to-r from-amber-100 to-yellow-200 border-amber-400' }; else if (exp >= 10) mastery = { label: '💎 전문가', color: 'text-blue-700', bg: 'bg-blue-100 border-blue-300' };
    const weekKey = getWeekKey(); const attendedDays = safeArray(db.attendance?.[weekKey]?.[s.id]); const weeklyCount = attendedDays.length; const streak = db.streakWeeks?.[s.id] || 0; const todayIdx = getTodayWeekdayIdx(); const attendedToday = todayIdx >= 0 && attendedDays.includes(todayIdx);
    return { ...s, exp, coins, mastery, status: db.studentStatus[s.id] || 'normal', atExp: db.allTime?.exp?.[s.id] || 0, atDonate: db.allTime?.donate?.[s.id] || 0, atFund: db.allTime?.fund?.[s.id] || 0, atPen: db.allTime?.penalty?.[s.id] || 0, weeklyCount, streak, attendedToday, notes: safeArray(db.notes?.[s.id]) };
  ...
  const { classReputation, shieldPoints, evolutionLevel, progressPercent } = useMemo(() => {
    const target = db.settings?.targetScore || 5000; const penaltyUnit = db.settings?.pointConfig?.penalty || 20;
    const raw = allStats.reduce((sum, s) => sum + (s.exp * 10) + (db.bonusCoins?.[s.id] || 0) - ((db.penaltyCount[s.id] || 0) * penaltyUnit), 0) + safeArray(db.donations).reduce((sum, d) => sum + (d.amount || 0), 0) + (db.manualRepOffset || 0);
    let r = Math.max(0, raw); let s = db.shieldPoints || 0; if (raw > target) { r = target; s = raw - target; }
    const step = Math.max(1, target / 5); const level = Math.min(Math.floor(r / step), 5); const pct = level >= 5 ? 100 : ((r % step) / step) * 100;
  ...
  const sortedDashboardStats = useMemo(() => { if (db.settings?.showCumulativeStats) return [...allStats].sort((a,b) => a.id - b.id); const order = { crisis: 0, pending: 1, normal: 2 }; return [...allStats].sort((a,b) => order[a.status] !== order[b.status] ? order[a.status] - order[b.status] : a.id - b.id); }, [allStats, db.settings?.showCumulativeStats]);
  ...
  // ── HANDLERS ──
  const toggleAttendance = (sId) => { const todayIdx = getTodayWeekdayIdx(); if (todayIdx < 0) return alert("주말에는 출석 불가."); const weekKey = getWeekKey(); const currentDb = dbRef.current; const weekData = currentDb.attendance?.[weekKey] || {}; const days = safeArray(weekData[sId]); const alreadyToday = days.includes(todayIdx); if (!alreadyToday && !isAttendanceOpen()) return alert("출석 시간이 지났어요. (08:30 마감)"); const newDays = alreadyToday ? days.filter(d => d !== todayIdx) : [...days, todayIdx].sort(); const newWeekData = { ...weekData, [sId]: newDays }; const updates = { attendance: { ...currentDb.attendance, [weekKey]: newWeekData }, manualRepOffset: (currentDb.manualRepOffset || 0) + (alreadyToday ? -1 : 1) }; if (!alreadyToday) { playSound('attend'); setAttendAnim({ id: sId, ts: Date.now() }); setTimeout(() => setAttendAnim(null), 1400); const totalDays = newDays.length + (currentDb.extraAttendDays || 0); const alreadyBonus = currentDb.attendanceBonus?.[weekKey]?.[sId]; if (totalDays >= 5 && !alreadyBonus) { updates.bonusCoins = { ...currentDb.bonusCoins, [sId]: (currentDb.bonusCoins?.[sId] || 0) + 3 }; updates.attendanceBonus = { ...currentDb.attendanceBonus, [weekKey]: { ...(currentDb.attendanceBonus?.[weekKey] || {}), [sId]: true } }; updates.streakWeeks = { ...currentDb.streakWeeks, [sId]: (currentDb.streakWeeks?.[sId] || 0) + 1 }; setTimeout(() => playSound('jackpot'), 600); setTimeout(() => alert(`🎉 ${allStats.find(x=>x.id===sId)?.name} 개근 달성! 🪙3 보너스!`), 900); } } sync(updates); };
  const teacherAddHoliday = () => { if (!window.confirm("공휴일 반영할까요?")) return; const currentDb = dbRef.current; const next = (currentDb.extraAttendDays || 0) + 1; const weekKey = getWeekKey(); const updates = { extraAttendDays: next }; const weekData = currentDb.attendance?.[weekKey] || {}; let newBonus = { ...(currentDb.attendanceBonus?.[weekKey] || {}) }; let newStreaks = { ...(currentDb.streakWeeks || {}) }; let newCoins = { ...(currentDb.bonusCoins || {}) }; let bonusCount = 0; safeArray(currentDb.students).forEach(s => { const days = safeArray(weekData[s.id]); const total = days.length + next; if (total >= 5 && !newBonus[s.id]) { newBonus[s.id] = true; newStreaks[s.id] = (newStreaks[s.id] || 0) + 1; newCoins[s.id] = (newCoins[s.id] || 0) + 3; bonusCount++; } }); updates.attendanceBonus = { ...currentDb.attendanceBonus, [weekKey]: newBonus }; updates.streakWeeks = newStreaks; updates.bonusCoins = newCoins; sync(updates); alert(`📅 보정 완료 (+${next}일, 개근 ${bonusCount}명)`); };
  const openNoteModal = (sId) => { if (isAuthenticated !== 'teacher') return setShowModal('password'); setShowNoteModal(sId); setNoteText(""); };
  const submitNote = () => { if (!noteText.trim()) return alert("입력하세요."); const sId = showNoteModal; const note = { id: Date.now(), date: formatDate(), text: noteText.trim() }; sync({ notes: { ...db.notes, [sId]: [...safeArray(db.notes?.[sId]), note] } }); setShowNoteModal(null); setNoteText(""); alert("저장 완료!"); };
  ...
  const pauseTimer = () => { const t = db.timer || {}; if (!t.isRunning) return; if (t.mode === 'class_sw') { const elapsed = Date.now() - t.startedAt; sync({ timer: { ...t, isRunning: false, pausedElapsed: elapsed } }); } else if (t.mode === 'class_cd' || t.mode === 'break') { const remaining = t.endTime - Date.now(); sync({ timer: { ...t, isRunning: false, pausedRemaining: Math.max(0, remaining) } }); } };
  ...
  const revokeAllSessions = () => { if (!window.confirm("모든 기기에서 강제 로그아웃합니다. 본 기기도 로그아웃됩니다.")) return; sync({ settings: { ...db.settings, authRevokedAt: Date.now() } }); clearAuth(); setIsAuthenticated(false); setActiveTab('dashboard'); alert("모든 세션을 무효화했습니다."); };
  ...
  const factoryReset = () => { if (window.prompt("'초기화' 입력:") !== "초기화") return; sync({ roleExp: {}, bonusCoins: {}, usedCoins: {}, penaltyCount: {}, studentStatus: {}, pendingReflections: [], pendingPraises: [], approvedPraises: [], donations: [], pendingShopItems: [], shopItems: [], funding: [], manualRepOffset: 0, shieldPoints: 0, allTime: { exp: {}, penalty: {}, donate: {}, fund: {} }, timeAttack: { isActive: false, title: "", rewardRep: 100, endTime: null, cleared: [] }, attendance: {}, attendanceBonus: {}, streakWeeks: {}, notes: {}, extraAttendDays: 0 }); alert("전체 리셋 완료."); };
                ...
                return (
                  <div key={s.id} className={`p-8 rounded-[40px] border-4 shadow-sm transition-all relative flex flex-col bg-white hover:shadow-xl hover:-translate-y-1 ${s.status === 'crisis' ? 'border-slate-300 bg-slate-100 opacity-60 grayscale' : (s.status === 'pending' ? 'border-orange-300 bg-orange-50' : 'border-white hover:border-amber-300')}`}>
                    ...
                    <div className="flex flex-col mb-6 gap-4">
                      <p className="text-[13px] font-bold text-slate-400 tracking-wide truncate bg-slate-50 inline-block w-max px-3 py-1 rounded-md">{s.group}모둠 · {s.role}</p>
                      ...
                      >
                        <h3 className={`text-3xl font-black flex items-center gap-2 tracking-tight truncate ${s.exp >= 20 && s.status !== 'crisis' ? 'text-amber-700' : nameColor}`}>
                          ...
                          <span className="text-sm font-bold text-slate-400 ml-2 bg-white px-2 py-0.5 rounded-md border">{s.weeklyCount}/5 · ⭐{s.streak}</span>
                        </h3>
                      </button>
                    ...
                    <div className="mt-auto pt-3">
                      {s.status === 'normal' && <button onClick={() => handleGivePenalty(s.id)} className="w-full py-4 bg-slate-50 border border-slate-200 hover:bg-red-50 text-slate-400 hover:text-red-600 hover:border-red-200 rounded-[20px] font-black text-sm flex items-center justify-center gap-2 transition-all"><AlertTriangle className="w-5 h-5"/> 위기 상태로 지정</button>}
                      {s.status === 'crisis' && <p className="text-center font-black text-white bg-slate-600 py-4 rounded-[20px] text-base shadow-md flex items-center justify-center gap-2">🚨 성찰과 회복 요망</p>}
                      {s.status === 'pending' && <p className="text-center font-black text-orange-800 bg-orange-200 py-4 rounded-[20px] text-base shadow-md flex items-center justify-center gap-2">⏳ 교사 승인 대기중</p>}
                    </div>
                  </div>
                );
              })}
              ...
              <div className="mb-12 bg-gradient-to-r from-yellow-100 to-amber-100 p-8 rounded-[40px] shadow-sm flex flex-col md:flex-row gap-8 items-center border-2 border-yellow-200">
                ...
                <div className="md:w-2/3 flex gap-4 w-full">
                  ...
                  <button onClick={() => {
                    ...
                    if (!sid || !amt) return alert("정보를 모두 입력하세요."); handleDonate(toInt(sid), amt);
                  }} className="bg-amber-500 text-white px-8 rounded-2xl font-black text-xl hover:bg-amber-600 active:scale-95 shadow-md">기부</button>
                </div>
              </div>
              ...
              {helpSubTab === 'shop' && (
                <div className="space-y-10 animate-in fade-in">
                  ...
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-10 rounded-[40px] border-4 border-amber-200 shadow-sm relative overflow-hidden">
                    ...
                    <p className="text-base font-bold text-amber-700 mb-8 relative z-10">숙련도 20회 이상 '장인'의 기발한 아이템을 기획해 결재를 올립니다.</p>
                      ...
                      <div key={item.id} className="bg-white p-10 rounded-[40px] shadow-sm border-2 border-slate-100 flex flex-col justify-between hover:border-amber-300 transition-colors">
                        ...
                        <div className="flex gap-4">
                          ...
                          <button onClick={() => {
                            ...
                            if (!window.confirm(`${user.name}의 코인을 차감할까요?`)) return;
                            sync({ usedCoins: { ...db.usedCoins, [sid]: (db.usedCoins[sid] || 0) + toInt(item.price) } });
                            alert("결제 승인 완료!"); playSound('buy');
                      ...
                      <div key={f.id} className="bg-gradient-to-br from-blue-500 to-indigo-600 p-10 rounded-[40px] shadow-xl text-white flex flex-col justify-between relative overflow-hidden border-4 border-blue-400">
                        ...
                        <div className="flex gap-4 relative z-10">
                          ...
                          <button onClick={() => {
                            ...
                            if (!sid || !amt) return alert("정확히 입력하세요."); handleFund(f.id, toInt(sid), amt);
                          }} className="bg-yellow-400 text-yellow-900 px-10 rounded-2xl font-black text-xl shadow-lg hover:bg-yellow-300 active:scale-95 transition-transform">투자</button>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {activeTab === 'admin' && isAuthenticated === 'teacher' && (
          <div className="bg-white rounded-[50px] shadow-sm border border-slate-100 flex flex-col lg:flex-row min-h-[850px] animate-in fade-in overflow-hidden">
            <aside className="w-full lg:w-80 bg-slate-900 p-10 flex flex-col gap-4 shrink-0 border-r border-slate-800">
              ...
              <button onClick={handleLogout} className="mt-auto p-5 bg-slate-800 text-slate-400 font-black rounded-2xl text-center hover:bg-slate-700 flex items-center justify-center gap-2 transition-colors">
              ...
              {adminSubTab === 'mission' && (
                <div className="space-y-10 animate-in fade-in">
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-200">
                    ...
                    <p className="text-sm font-bold text-slate-500 mb-8 bg-slate-50 p-4 rounded-2xl border border-slate-100">이곳에서 미션 이름과 보상 점수를 변경하세요.</p>
                      ...
                      {[
                        ...
                        <div key={q.key} className={`${q.bgClass} p-4 rounded-2xl border flex gap-3 shadow-sm`}>
                          <input type="text" value={db.coopQuest?.[`${q.key}Name`] || q.defName}
                                 ...
                                 onFocus={lockEditing} onBlur={unlockEditing} className="flex-1 p-3 rounded-xl text-sm font-bold border-none outline-none shadow-sm"/>
                          <input type="number" value={db.coopQuest?.[q.key] ?? q.defVal}
                                 onChange={e => sync({ coopQuest: { ...db.coopQuest, [q.key]: toInt(e.target.value) } })}
                                 onFocus={lockEditing} onBlur={unlockEditing} className={`w-20 p-3 rounded-xl text-base font-black ${q.textClass} border-none outline-none text-center shadow-sm`}/>
                        </div>
                      ))}
                    </div>
                      ...
                      {db.timeAttack?.isActive ? (
                        <div className="space-y-6">
                          ...
                          <div className="flex gap-3">
                            <button onClick={handleCompleteTimeAttack} className="flex-1 bg-green-500 text-white py-4 rounded-2xl font-black text-lg shadow-md hover:bg-green-600 active:scale-95 transition-transform">미션 성공 승인</button>
                            <button onClick={handleFailTimeAttack} className="flex-1 bg-slate-400 text-white py-4 rounded-2xl font-black text-lg shadow-md hover:bg-slate-500 active:scale-95 transition-transform">실패 종료</button>
                          </div>
                        </div>
                      ) : (
                        <div className="space-y-4">
                          <div className="bg-white p-6 rounded-2xl border border-red-200 space-y-5 shadow-sm">
                            <div>
                              ...
                              <input type="text" placeholder="예: 바닥 쓰레기 0개!" value={taTitle} onChange={e=>setTaTitle(e.target.value)} onFocus={lockEditing} onBlur={unlockEditing} className="w-full p-4 rounded-xl border border-red-100 font-bold outline-none focus:border-red-400"/>
                            </div>
                            <div className="flex gap-4">
                              <div className="flex-1">
                                <label className="block text-[11px] font-black text-red-400 mb-1 ml-1">제한 시간(분)</label>
                                <input type="number" value={taMins} onChange={e=>setTaMins(e.target.value)} onFocus={lockEditing} onBlur={unlockEditing} className="w-full p-4 rounded-xl border border-red-100 font-black text-center outline-none focus:border-red-400"/>
                              </div>
                              <div className="flex-1">
                                <label className="block text-[11px] font-black text-red-400 mb-1 ml-1">성공 보상(p)</label>
                                <input type="number" value={taReward} onChange={e=>setTaReward(e.target.value)} onFocus={lockEditing} onBlur={unlockEditing} className="w-full p-4 rounded-xl border border-red-100 font-black text-center outline-none focus:border-red-400"/>
                              </div>
                            </div>
                          </div>
                    ...
                    <div className="w-full space-y-5 max-h-[500px] overflow-y-auto pr-4 custom-scrollbar">
                      {safeArray(db.pendingShopItems).filter(i => i && i.name).map(item => (
                        <div key={item.id} className="bg-amber-50 p-6 rounded-[24px] border-2 border-amber-200 text-left shadow-sm">
                          ...
                          <div className="flex gap-2">
                            <button onClick={() => { sync({ pendingShopItems: safeArray(db.pendingShopItems).filter(i => i.id !== item.id), shopItems: [item, ...safeArray(db.shopItems)] }); alert("상점 등록 완료!"); playSound('good'); }} className="flex-1 bg-amber-500 text-white py-3 rounded-xl font-black text-sm hover:bg-amber-600 shadow-md">상점에 출시 허가</button>
                            <button onClick={() => { sync({ pendingShopItems: safeArray(db.pendingShopItems).filter(i => i.id !== item.id) }); alert("반려"); }} className="px-4 bg-white text-slate-400 font-black rounded-xl border border-slate-200">반려</button>
                          </div>
                        </div>
                      ))}
                      {safeArray(db.pendingReflections).filter(r => r && r.sId).map(r => (
                        <div key={r.id} className="bg-red-50 p-6 rounded-[24px] border-2 border-red-200 text-left shadow-sm">
                          ...
                          <div className="flex gap-3">
                            <button onClick={() => { sync({ pendingReflections: safeArray(db.pendingReflections).filter(pr => pr.id !== r.id), studentStatus: { ...db.studentStatus, [r.sId]: 'normal' } }); alert("위기 해제 승인 완료!"); playSound('good'); }} className="flex-1 bg-red-500 text-white py-4 rounded-xl font-black text-base hover:bg-red-600 shadow-md active:scale-95 transition-transform">위기 해제 및 복귀 승인</button>
                            <button onClick={() => { sync({ pendingReflections: safeArray(db.pendingReflections).filter(pr => pr.id !== r.id), studentStatus: { ...db.studentStatus, [r.sId]: 'crisis' } }); alert("반려"); }} className="px-6 bg-white text-slate-500 font-black rounded-xl border-2 border-slate-200 hover:bg-slate-50">다시 성찰하기 (반려)</button>
                          </div>
                        </div>
                      ))}
                      {safeArray(db.pendingPraises).filter(p => p && p.toId).map(p => {
                        ...
                        return (
                          <div key={p.id} className="bg-pink-50 p-6 rounded-[24px] border-2 border-pink-200 text-left shadow-sm">
                            ...
                            <div className="flex gap-3">
                              <button onClick={() => approvePraise(p)} className={`flex-1 py-4 rounded-xl font-black text-base shadow-md transition-all ${isCrisis ? 'bg-slate-300 text-slate-500 cursor-not-allowed' : 'bg-pink-500 text-white hover:bg-pink-600 active:scale-95'}`}>온기 사연 승인</button>
                              <button onClick={() => { sync({ pendingPraises: safeArray(db.pendingPraises).filter(pr => pr.id !== p.id) }); alert("반려되었습니다."); }} className="px-6 bg-white text-slate-500 font-black rounded-xl border-2 border-slate-200 hover:bg-slate-50">반려</button>
                            </div>
                          </div>
                        );
                      })}
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 space-y-8">
                    ...
                    <div className="pt-6">
                      ...
                      <div className="flex gap-3 mb-6">
                        <input type="text" placeholder="새로운 물품 이름" value={newItemName} onChange={e=>setNewItemName(e.target.value)} onFocus={lockEditing} onBlur={unlockEditing} className="flex-1 p-4 rounded-xl border border-slate-300 font-bold outline-none"/>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {safeArray(db.shopItems).filter(i => i && i.name).map(item => (
                          <div key={item.id} className="bg-amber-50 p-4 rounded-2xl border border-amber-200 flex justify-between items-center shadow-sm">
                            <div><span className="text-[10px] text-slate-400 font-black bg-white px-2 py-1 rounded-md">{String(item.creator)}</span><h4 className="font-black text-slate-800 mt-2">{String(item.name)}</h4><p className="text-amber-600 font-black text-sm">{toInt(item.price)}🪙</p></div>
                    ...
                    <div className="pt-8 border-t border-slate-200 mt-8">
                      ...
                      <div className="flex gap-3 mb-6 bg-blue-50 p-6 rounded-3xl border border-blue-100">
                        <input type="text" placeholder="새로운 펀딩 목표 (예: 피자 파티)" value={newFundName} onChange={e=>setNewFundName(e.target.value)} onFocus={lockEditing} onBlur={unlockEditing} className="flex-1 p-4 rounded-xl border border-blue-200 font-bold outline-none focus:border-blue-400"/>
                        <input type="number" placeholder="목표 점수" value={newFundTarget} onChange={e=>setNewFundTarget(e.target.value)} onFocus={lockEditing} onBlur={unlockEditing} className="w-32 p-4 rounded-xl border border-blue-200 font-bold outline-none focus:border-blue-400 text-center"/>
                        <button onClick={() => {
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {safeArray(db.funding).filter(f => f && f.name).map(f => (
                          <div key={f.id} className="bg-white p-5 rounded-2xl border border-blue-200 flex justify-between items-center shadow-sm">
                            <div><h4 className="font-black text-blue-900 text-lg">{String(f.name)}</h4><p className="text-blue-500 font-bold text-sm mt-1">현재: {toInt(f.current)} / 목표: {toInt(f.target)}p</p></div>
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 flex flex-col md:flex-row gap-10">
                    <div className="w-full md:w-1/3 space-y-4">
                      ...
                      <div className="bg-indigo-50 p-6 rounded-3xl border border-indigo-100 space-y-3 shadow-sm">
                        ...
                        <button onClick={exportAll} className="w-full p-4 rounded-xl font-black text-sm bg-purple-500 text-white hover:bg-purple-600 flex items-center justify-center gap-2 shadow-md">
                          <Download className="w-4 h-4"/> 전체 반 추출
                        </button>
                        <p className="text-[11px] font-bold text-indigo-400 leading-relaxed mt-2">클립보드에 자동 복사되며 파일로도 다운로드됩니다. ChatGPT/Claude에 그대로 붙여넣으세요.</p>
                      </div>
                      {selectedReportStudent && (
                        <button onClick={() => setShowRollingPaper(toInt(selectedReportStudent))} className="w-full bg-gradient-to-r from-pink-400 to-rose-500 text-white p-5 rounded-3xl font-black text-lg shadow-lg flex items-center justify-center gap-3 mt-6 transition-transform hover:-translate-y-1">
                        ...
                        return (
                          <div className="animate-in fade-in zoom-in-95">
                            ...
                            <div className="border-t-2 border-slate-200 pt-8 mt-8">
                              <button onClick={() => setShowNotesInReport(!showNotesInReport)} className="w-full bg-yellow-100 hover:bg-yellow-200 text-yellow-800 font-black p-4 rounded-xl flex items-center justify-between transition-colors shadow-sm">
                                <span className="flex items-center gap-3"><StickyNote className="w-6 h-6"/> 누가기록 확인 ({s.notes.length}건)</span><span>{showNotesInReport ? '▲ 접기' : '▼ 펼치기'}</span>
                              </button>
                              {showNotesInReport && (
                                <div className="mt-4 space-y-3 animate-in fade-in">
                                  ...
                                  {s.notes.map(n => (
                                    <div key={n.id} className="bg-white p-4 rounded-2xl border border-yellow-200 flex justify-between items-start gap-4 shadow-sm">
                                      <div><p className="text-[11px] font-black text-yellow-600 mb-1">{n.date}</p><p className="text-base font-bold text-slate-700 whitespace-pre-wrap leading-relaxed">{n.text}</p></div>
                                      <button onClick={() => deleteNote(s.id, n.id)} className="p-2.5 bg-red-50 text-red-500 rounded-xl shrink-0 hover:bg-red-100"><Trash2 className="w-4 h-4"/></button>
                                    </div>
                                  ))}
                                  <button onClick={() => openNoteModal(s.id)} className="w-full bg-yellow-500 text-white p-4 rounded-2xl font-black text-base flex items-center justify-center gap-2 hover:bg-yellow-600 shadow-md mt-4"><Plus className="w-5 h-5"/> 이 학생의 새 기록 추가</button>
                                </div>
                              )}
                            </div>
                          </div>
                ...
                <div className="space-y-8 animate-in fade-in">
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100">
                    <div className="flex flex-wrap gap-4 mb-10 bg-slate-50 p-8 rounded-[30px] border border-slate-200 shadow-sm">
                      ...
                      <select value={newStudentGroup} onChange={e=>setNewStudentGroup(e.target.value)} className="w-36 p-5 rounded-2xl border border-slate-300 font-bold outline-none text-lg shadow-sm">
                        {[1,2,3,4,5,6].map(g => <option key={g} value={g}>{g}모둠</option>)}
                      </select>
                      <select value={newStudentEnneagram} onChange={e=>setNewStudentEnneagram(e.target.value)} className="w-40 p-5 rounded-2xl border border-slate-300 font-bold outline-none text-lg shadow-sm">
                ...
                <div className="space-y-8 animate-in fade-in">
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100">
                    <div className="bg-amber-50 p-8 rounded-3xl border-2 border-amber-200 mb-10 shadow-sm">
                      ...
                      <button onClick={teacherAddHoliday} className="w-full bg-amber-500 text-white py-5 rounded-2xl font-black text-xl hover:bg-amber-600 shadow-md transition-transform active:scale-95">전체 학생의 출석일수 +1일 반영하기</button>
                ...
                <div className="space-y-8 animate-in fade-in">
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 space-y-10">
                    ...
                    <div className="bg-slate-50 p-10 rounded-[30px] border border-slate-200 grid grid-cols-1 md:grid-cols-2 gap-10">
                      {[
                        ...
                        <div key={p.field}>
                          ...
                          <div className="flex gap-3">
                            <input type="password" value={p.val} onChange={e=>p.setter(e.target.value)} onFocus={lockEditing} onBlur={unlockEditing} placeholder="새 비밀번호 입력" className="flex-1 p-4 rounded-2xl border border-slate-300 font-black outline-none focus:border-blue-400 shadow-sm text-lg"/>
                            <button onClick={() => { if (!p.val) return alert('입력하세요.'); sync({ settings: { ...db.settings, [p.field]: p.val } }); alert('변경 완료!'); p.setter(''); }} className={`bg-${p.color}-${p.color==='blue'?'600':'500'} text-white px-6 rounded-2xl font-black text-lg shadow-md`}>변경</button>
                          </div>
                    ...
                    <div className="bg-red-50 p-6 rounded-3xl border-2 border-red-200">
                      ...
                      <button onClick={revokeAllSessions} className="w-full bg-red-600 text-white py-4 rounded-2xl font-black text-lg shadow-md hover:bg-red-700 transition-colors">모든 세션 즉시 무효화</button>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t-2 border-slate-100">
                      <div>
                        ...
                        <input type="text" value={db.settings?.title || ""} onChange={e=>sync({ settings: { ...db.settings, title: e.target.value } })} onFocus={lockEditing} onBlur={unlockEditing} className="w-full p-6 rounded-3xl bg-slate-50 border border-slate-200 font-black text-xl outline-none focus:border-blue-400 shadow-sm"/>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <input type="number" value={Math.floor((db.settings?.defaultBreakMs || DEFAULT_BREAK_MS)/60000)} onChange={e => sync({ settings: { ...db.settings, defaultBreakMs: toInt(e.target.value, 10)*60000 } })} onFocus={lockEditing} onBlur={unlockEditing} className="w-full p-6 rounded-3xl bg-slate-50 border border-slate-200 font-black text-xl outline-none focus:border-blue-400 shadow-sm"/>
                      </div>
                    </div>
                    ...
                    <div className="pt-8 border-t-2 border-slate-100">
                      ...
                      <button onClick={toggleCumulativeStats} className={`w-full py-5 rounded-2xl font-black text-xl transition-all shadow-md flex items-center justify-center gap-3 ${db.settings?.showCumulativeStats ? 'bg-blue-600 text-white' : 'bg-white text-slate-500 border-2 border-slate-300 hover:border-blue-300'}`}>
                    ...
                    <div className="pt-8 border-t-2 border-slate-100 bg-indigo-50/50 p-10 rounded-[40px] border border-indigo-100">
                      ...
                      <div className="flex flex-col md:flex-row gap-4 mb-10 bg-white p-6 rounded-3xl shadow-sm border border-indigo-50">
                        <input type="number" value={manualScoreInput} onChange={e=>setManualScoreInput(e.target.value)} onFocus={lockEditing} onBlur={unlockEditing} placeholder="명성 점수 강제 추가/차감 (예: +50 또는 -20)" className="flex-1 p-4 rounded-2xl border border-slate-200 font-black outline-none text-base focus:border-indigo-400"/>
                        <button onClick={() => { const v = toInt(manualScoreInput); if (!v) return; if (window.confirm(`${v}점 적용?`)) { sync({ manualRepOffset: (db.manualRepOffset||0) + v }); setManualScoreInput(""); } }} className="bg-indigo-600 text-white px-10 rounded-2xl font-black text-lg shadow-md hover:bg-indigo-700 active:scale-95 transition-transform">즉시 적용</button>
                      </div>
                        ...
                        {[
                          ...
                          <div key={f.key} className="bg-white p-5 rounded-2xl shadow-sm">
                            ...
                            <input type="number" value={f.section ? (db.settings?.[f.section]?.[f.key] ?? f.def) : (db.settings?.[f.key] ?? f.def)}
                                   ...
                                   onFocus={lockEditing} onBlur={unlockEditing} className={`w-full p-4 rounded-xl border border-${f.color}-200 font-black text-lg text-${f.color}-600 outline-none focus:border-${f.color}-400`}/>
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-[5001] p-4">
          <div className="bg-white p-12 rounded-[50px] w-full max-w-xl shadow-2xl animate-in zoom-in-95 border-4 border-pink-100">
            ...
            <div className="space-y-6 mb-10">
              <select value={praiseTarget} onChange={e=>setPraiseTarget(e.target.value)} className="w-full p-6 rounded-3xl bg-slate-50 border-2 font-black text-xl outline-none focus:border-pink-400 shadow-sm">
              ...
              <select value={praiseTag} onChange={e=>setPraiseTag(e.target.value)} className="w-full p-6 rounded-3xl bg-slate-50 border-2 font-black text-xl outline-none focus:border-pink-400 shadow-sm">
                <option value="">어떤 역량인가요?</option>
                {SEL_OPTIONS.map(opt => <option key={opt.name} value={opt.name}>{opt.name}</option>)}
              </select>
              <textarea value={praiseText} onChange={e=>setPraiseText(e.target.value)} rows="5" placeholder={praiseTag ? PRAISE_GUIDES[praiseTag] : "위에서 역량을 먼저 선택해 주세요! 💌"} className="w-full p-6 rounded-[30px] bg-pink-50 border-2 border-pink-100 font-black text-lg outline-none focus:border-pink-400 text-pink-900 shadow-inner resize-none placeholder:text-pink-300"/>
            </div>
            <div className="flex gap-4">
        ...
        return (
          <div className="fixed inset-0 bg-white z-[5004] overflow-auto flex flex-col items-center">
            <div className="w-full bg-slate-100 p-5 flex justify-between items-center print:hidden sticky top-0 z-50 border-b-2 shadow-sm">
              ...
              <div className="flex gap-3">
                <button onClick={() => window.print()} className="bg-blue-600 text-white px-8 py-3 rounded-2xl font-black flex items-center gap-2 shadow-md hover:bg-blue-700 active:scale-95 transition-transform"><Printer className="w-5 h-5"/> 인쇄하기</button>
